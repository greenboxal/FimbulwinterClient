...
//     <id value="$Id: ILImageCodec.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Plugins.DevILCodecs
{
    ...
    public class ILImageCodec : ImageCodec
    {
        ...
        public override void EncodeToFile(Stream input, string outFileName, Codec.CodecData codecData)
        {
            ...
            ImageData imgData = (ImageData) codecData;
            ...
            {
                LogManager.Instance.Write("IL Error, could not save file: {0} : {1}", outFileName,
                                          Ilu.iluErrorString(error));
            }
        ...
        public override Codec.DecodeResult Decode(Stream input)
        {
            ...
            {
                throw new AxiomException("Unsupported devil format ImageFormat={0} ImageType={1}", imageFormat,
                                         imageType);
            }
            ...
            for (int mip = 0; mip <= imgData.numMipMaps; ++mip)
            {
                for (int i = 0; i < numFaces; ++i)
                {
                    ...
                    if ((imgData.flags & ImageFlags.Compressed) != 0)
                    {
                        ...
                        {
                            LogManager.Instance.Write(
                                "Warning: compressed image size mismatch, devilsize={0} oursize={1}",
                    ...
                    else
                    {
                        ...
                        using (BufferHandle = BufferBase.Wrap(buffer))
                        {
                            PixelBox dst = new PixelBox(width, height, depth, imgData.format, BufferHandle);
