...
//     <id value="$Id: OpenTKGLSupport.cs 3322 2012-07-16 20:58:00Z borrillis $"/>
// </file>
        ...
        private void _refreshConfig()
        {
            ...
            for (int q = 0; q < dev.AvailableResolutions.Count; q++)
            {
                ...
                if (item.Width == width && item.Height == height && item.BitsPerPixel >= 16)
                {
                    ...
                    {
                        optColorDepth.PossibleValues.Add(optColorDepth.PossibleValues.Values.Count,
                                                         item.BitsPerPixel.ToString());
                    }
