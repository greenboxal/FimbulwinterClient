...
//     <id value="$Id: GLSLLinkProgramManager.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.RenderSystems.OpenGL.GLSL
{
    ...
    public sealed class GLSLLinkProgramManager : IDisposable
    {
        ...
        public GLSLLinkProgram ActiveLinkProgram
        {
            get
            {
                ...
                if (activeKey > 0)
                {
                    ...
                    {
                        this.activeLinkProgram = new GLSLLinkProgram(this.activeVertexProgram,
                                                                     this.activeGeometryProgram,
        ...
        private void CompleteDefInfo(int gltype, GpuProgramParameters.GpuConstantDefinition defToUpdate)
        {
            ...
            // GL doesn't pad
            defToUpdate.ElementSize = GpuProgramParameters.GpuConstantDefinition.GetElementSize(
                defToUpdate.ConstantType, false);
        }
        ...
        ///<param name="vertexConstantDefs"> Definition of the constants extracted from the vertex program, used to match up physical buffer indexes with program uniforms. May be null if there is no vertex program. </param>
        ///<param name="geometryConstantDefs"> Definition of the constants extracted from the geometry program, used to match up physical buffer indexes with program uniforms. May be null if there is no geometry program. </param>
        ///<param name="fragmentConstantDefs"> Definition of the constants extracted from the fragment program, used to match up physical buffer indexes with program uniforms. May be null if there is no fragment program. </param>
                                    ...
                                    GLSLLinkProgram.UniformReferenceList list)
        {
            ...
            Gl.glGetObjectParameterivARB(programObject, Gl.GL_OBJECT_ACTIVE_UNIFORMS_ARB, out uniformCount);
            ...
            for (int index = 0; index < uniformCount; index++)
            {
                ...
                GLSLLinkProgram.UniformReference newGLUniformReference = new GLSLLinkProgram.UniformReference();

                StringBuilder uniformName = new StringBuilder();
                int arraySize;
                int glType;
                Gl.glGetActiveUniformARB(programObject, index, BUFFERSIZE, null, out arraySize, out glType, uniformName);
                ...
                if (newGLUniformReference.Location >= 0)
                {
                    ...
                    bool foundSource = CompleteParamSource(paramName, vertexConstantDefs, geometryConstantDefs,
        ...
        ///<param name="defs"> The defs to populate (will not be cleared before adding, clear it yourself before calling this if that's what you want). </param>
        ...
        public void ExtractConstantDefs(String src, GpuProgramParameters.GpuNamedConstants defs, String filename)
        {
            ...
            while (currPos != -1)
            {
                GpuProgramParameters.GpuConstantDefinition def = new GpuProgramParameters.GpuConstantDefinition();
                string paramName = "";
                ...
                if (!inLargerString)
                {
                    ...
                    foreach (string _i in parts)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                LogManager.Instance.Write("Problem parsing the following GLSL Uniform: '" + line +
                                                          "' in file " + filename);
                                // next uniform
                                break;
                            }
