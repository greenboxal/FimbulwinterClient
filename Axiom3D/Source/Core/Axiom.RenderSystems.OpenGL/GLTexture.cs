...
//     <id value="$Id: GLTexture.cs 3322 2012-07-16 20:58:00Z borrillis $"/>
// </file>
...
namespace Axiom.RenderSystems.OpenGL
{
    ...
    public class GLTexture : Texture
    {
        ...
        protected void GenerateMipMaps(byte[] data, bool useSoftware, bool isCompressed, int faceNum)
        {
            ...
            int type = (TextureType == TextureType.CubeMap)
                           ? Gl.GL_TEXTURE_CUBE_MAP_POSITIVE_X + faceNum
                           : GLTextureType;
            ...
            if (useSoftware && MipmapCount > 0)
            {
                ...
                {
                    Glu.gluBuild1DMipmaps(type, HasAlpha ? Gl.GL_RGBA8 : Gl.GL_RGB8, Width, GLFormat,
                                          Gl.GL_UNSIGNED_BYTE, data);
                }
                else if (TextureType == TextureType.ThreeD)
                {
                    // TODO: Tao needs glTexImage3D
                    Gl.glTexImage3DEXT(type, 0, HasAlpha ? Gl.GL_RGBA8 : Gl.GL_RGB8, SrcWidth, SrcHeight, Depth, 0,
                                       GLFormat,
                                       Gl.GL_UNSIGNED_BYTE, data);
                }
                else
                {
                    // build the mipmaps
                    Glu.gluBuild2DMipmaps(type, HasAlpha ? Gl.GL_RGBA8 : Gl.GL_RGB8, Width, Height, GLFormat,
                                          Gl.GL_UNSIGNED_BYTE,
                                          data);
                }
            }
            else
            {
                if (TextureType == TextureType.OneD)
                {
                    Gl.glTexImage1D(type, 0, HasAlpha ? Gl.GL_RGBA8 : Gl.GL_RGB8, Width, 0, GLFormat,
                                    Gl.GL_UNSIGNED_BYTE, data);
                }
                else if (TextureType == TextureType.ThreeD)
                {
                ...
                {
                    if (isCompressed &&
                        Root.Instance.RenderSystem.Capabilities.HasCapability(Capabilities.TextureCompressionDXT))
                    {
                    ...
                    {
                        Gl.glTexImage2D(type, 0, HasAlpha ? Gl.GL_RGBA8 : Gl.GL_RGB8, Width, Height, 0, GLFormat,
                                        Gl.GL_UNSIGNED_BYTE,
                                        data);
                    }
                }
            ...
            for (int face = 0; face < FaceCount; face++)
            {
                for (int mip = 0; mip <= MipmapCount; mip++)
                {
                    GLHardwarePixelBuffer buf = new GLTextureBuffer(Name, GLTextureType, this._glTextureID, face, mip,
                                                                    (BufferUsage) Usage,
                                                                    doSoftware && mip == 0, this._glSupport,
                                                                    HardwareGammaEnabled,
                                                                    FSAA);
        ...
        protected override void createInternalResources()
        {
            ...
            mipmapsHardwareGenerated =
                Root.Instance.RenderSystem.Capabilities.HasCapability(Capabilities.HardwareMipMaps);
...

            {
                ...
                for (int mip = 0; mip <= MipmapCount; mip++)
                {
                    ...
                    {
                        case TextureType.OneD:
                            Gl.glTexImage1D(Gl.GL_TEXTURE_1D, mip, format, width, 0, Gl.GL_RGBA, Gl.GL_UNSIGNED_BYTE,
                                            IntPtr.Zero);

                            break;
                        case TextureType.TwoD:
                            Gl.glTexImage2D(Gl.GL_TEXTURE_2D, mip, format, width, height, 0, Gl.GL_RGBA,
                                            Gl.GL_UNSIGNED_BYTE, IntPtr.Zero);
                            break;
                        case TextureType.ThreeD:
                            Gl.glTexImage3D(Gl.GL_TEXTURE_3D, mip, format, width, height, depth, 0, Gl.GL_RGBA,
                                            Gl.GL_UNSIGNED_BYTE,
                                            IntPtr.Zero);
                            break;
                        case TextureType.CubeMap:
                            ...
                            {
                                Gl.glTexImage2D(Gl.GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, mip, format, width, height, 0,
                                                Gl.GL_RGBA,
                                                Gl.GL_UNSIGNED_BYTE, IntPtr.Zero);
                            }
                            break;
