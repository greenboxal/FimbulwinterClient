    ...
    internal class GLFrameBufferObject : DisposableObject
    {
        ...
        private void _initialize()
        {
            ...
            for (int x = 0; x < Config.MaxMultipleRenderTargets; ++x)
            {
                if (this._color[x].Buffer != null)
                {
                    if (this._color[x].Buffer.Width != width || this._color[x].Buffer.Height != height)
                    {
                        throw new ArgumentException(
                            String.Format(
                                "Attachment {0} has incompatible size {1}x{2}. It must be of the same as the size of surface 0, {3}x{4}.",
                                x,
                ...
                {
                    // Detach
                    Gl.glFramebufferRenderbufferEXT(Gl.GL_FRAMEBUFFER_EXT, Gl.GL_COLOR_ATTACHMENT0_EXT + x,
