...
//     <id value="$Id: OverlayElementManager.cs 3361 2012-07-29 00:06:09Z romeoxbm $"/>
// </file>
...
namespace Axiom.Overlays
{
    ...
    public sealed class OverlayElementManager : DisposableObject
    {
        ...
        {
            get { return this._elementInstances.Values; }
        }
        ...
        {
            get { return this._elementTemplates.Values; }
        }
        ...
        {
            this._elementFactories.Add(factory.Type, factory);
        ...
        {
            if (!this._elementFactories.ContainsKey(typeName))
            {
                throw new AxiomException("Cannot locate factory for element type '{0}'", typeName);
            }

            // create the element
            return (this._elementFactories[typeName]).Create(instanceName);
        }
        ...
        {
            return isTemplate ? this._elementTemplates : this._elementInstances;
        }
        ...
        {
            Dictionary<string, OverlayElement> elements = isTemplate ? this._elementTemplates : this._elementInstances;
            if (!elements.ContainsKey(name))
            {
...
#endif
        {
            Dictionary<string, OverlayElement> dict = (isTemplate ? this._elementTemplates : this._elementInstances);
            foreach (OverlayElement current in dict.Values)
                current.SafeDispose();
