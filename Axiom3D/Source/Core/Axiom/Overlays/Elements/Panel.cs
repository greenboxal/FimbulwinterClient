...
//     <id value="$Id: Panel.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
        ...
        protected override void UpdateTextureGeometry()
        {
            if (material != null && isInitialized)
            {
                ...
                else if (this.numTexCoordsInBuffer < numLayers)
                {
                    ...
                    for (int i = this.numTexCoordsInBuffer; i < numLayers; ++i)
                    {
                        decl.AddElement(TEXTURE_COORDS, offset, VertexElementType.Float2,
                                        VertexElementSemantic.TexCoords, i);
                ...
                if (this.numTexCoordsInBuffer != numLayers)
                {
                    HardwareVertexBuffer newBuffer =
                        HardwareBufferManager.Instance.CreateVertexBuffer(decl.Clone(TEXTURE_COORDS),
                ...
                if (this.numTexCoordsInBuffer != 0)
                {
                    ...
                    HardwareVertexBuffer buffer =
                        renderOperation.vertexData.vertexBufferBinding.GetBuffer(TEXTURE_COORDS);
                    BufferBase data = buffer.Lock(BufferLocking.Discard);
