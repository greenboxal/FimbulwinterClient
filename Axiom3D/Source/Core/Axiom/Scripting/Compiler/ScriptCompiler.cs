...
namespace Axiom.Scripting.Compiler
{
    ...
    public partial class ScriptCompiler
    {
        ...
        {
            get { return this._keywordMap; }
        }

        /// <summary>
        ///   The set of imported scripts to avoid circular dependencies
        /// </summary>
        private readonly Dictionary<string, IList<AbstractNode>> _imports =
            new Dictionary<string, IList<AbstractNode>>();
        ...
        public bool Compile(IList<AbstractNode> nodes, string group, bool doImports, bool doObjects, bool doVariables)
        {
            ...
            foreach (AbstractNode currentNode in nodes)
            {
                ...
                Translator translator = ScriptCompilerManager.Instance.GetTranslator(currentNode);
        ...
        internal void AddError(CompileErrorCode code, string file, uint line, string msg)
        {
            ...
            else
            {
                string str = string.Format("Compiler error: {0} in {1}({2})",
                                           ScriptEnumAttribute.GetScriptAttribute((int) code, typeof (CompileErrorCode)),
                                           file,
                                           line);
        ...
        private void _processImports(ref IList<AbstractNode> nodes)
        {
            ...
            foreach (KeyValuePair<string, IList<AbstractNode>> it in this._imports)
            {
                if (this._importRequests.ContainsKey(it.Key))
                {
                    string j = this._importRequests[it.Key];
        ...
        private void _processVariables(ref IList<AbstractNode> nodes)
        {
            for (int i = 0; i < nodes.Count; ++i)
            {
                ...
                else if (cur is VariableGetAbstractNode)
                {
                    ...
                    {
                        bool found = this._environment.ContainsKey(var.Name);
                        if (found)
                        {
        ...
        private IList<AbstractNode> _loadImportPath(string name)
        {
            ...
            {
                using (Stream stream = ResourceGroupManager.Instance.OpenResource(name, this._resourceGroup))
                {
                    if (stream != null)
                    {
                        ScriptLexer lexer = new ScriptLexer();
        ...
        private void _overlayObject(AbstractNode source, ObjectAbstractNode dest)
        {
            if (source is ObjectAbstractNode)
            {
                ...
                List<KeyValuePair<AbstractNode, AbstractNode>> overrides =
                    new List<KeyValuePair<AbstractNode, AbstractNode>>();
                ...
                for (int i = 0; i < dest.Children.Count; i++)
                {
                    if (dest.Children[i] is ObjectAbstractNode)
                    {
                        ...
                        for (int j = 0; j < overrides.Count; ++j)
                        {
                            ...
                            if (temp.Cls == node.Cls && !string.IsNullOrEmpty(node.Name) &&
                    ...
                    if (i is ObjectAbstractNode)
                    {
                        ...
                        if (!overridden[node])
                        {
                            ...
                            if (overrideIndex < overrides.Count)
                            {
                                ...
                                {
                                    ObjectAbstractNode temp = (ObjectAbstractNode) overrides[j].Key;
                                    if (string.IsNullOrEmpty(temp.Name) && temp.Cls == node.Cls &&
                                        overrides[j].Value == null)
                                    {
        ...
        private void InitializeWordMap()
        {
            ...
            this._keywordMap["shadow_receiver_vertex_program_ref"] =
                (uint) Keywords.ID_SHADOW_RECEIVER_VERTEX_PROGRAM_REF;
            this._keywordMap["shadow_receiver_fragment_program_ref"] =
                (uint) Keywords.ID_SHADOW_RECEIVER_FRAGMENT_PROGRAM_REF;
