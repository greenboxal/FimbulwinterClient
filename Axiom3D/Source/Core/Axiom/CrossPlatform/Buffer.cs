...
//     <id value="$Id: Buffer.cs 3361 2012-07-29 00:06:09Z romeoxbm $"/>
// </file>
...
namespace Axiom.Core
{
    ...
    public abstract class BufferBase : DisposableObject, ICloneable
    {
        ...
        public static BufferBase operator +(BufferBase buffer, int offset)
        {
            ...
            BufferBase buf = (BufferBase) buffer.Clone();
            buf.Ptr += offset;
            return buf;
        }
    ...
    public class ManagedBuffer
        : BufferBase, ITypePointer<byte>, ITypePointer<short>, ITypePointer<ushort>, ITypePointer<int>,
          ITypePointer<uint>,
          ITypePointer<long>, ITypePointer<ulong>, ITypePointer<float>, ITypePointer<double>
    {
        ...
        {
            get { return this.IdxPtr; }
            set { this.IdxPtr = value; }
        }
        ...
        public override IntPtr Pin()
        {
            if (Interlocked.Increment(ref PinCount) > 0)
            {
                lock (_pinMutex)
                {
                    return
                        new IntPtr(
                            (PinHandle.IsAllocated
                                 ? PinHandle
                                 : PinHandle = GCHandle.Alloc(this.Buf, GCHandleType.Pinned)).
    ...
    public class BitConvertBuffer
        : ManagedBuffer, ITypePointer<short>, ITypePointer<ushort>, ITypePointer<int>, ITypePointer<uint>,
          ITypePointer<long>,
