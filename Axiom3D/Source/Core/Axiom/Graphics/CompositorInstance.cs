...
namespace Axiom.Graphics
{
    ...
    public class CompositorInstance : DisposableObject
    {
        ...
        {
            get { return this.renderSystemOperations; }
            set { this.renderSystemOperations = value; }
        }
        ...
        {
            get { return this.previousInstance; }
            set { this.previousInstance = value; }
        }
            ...
            foreach (CompositionPass pass in target.Passes)
            {
                switch (pass.Type)
                {
                    case CompositorPassType.Clear:
                        {
                            QueueRenderSystemOp(finalState,
                                                new RSClearOperation(pass.ClearBuffers, pass.ClearColor, pass.ClearDepth,
                                                                     pass.ClearStencil));
                        }
                        break;
                    case CompositorPassType.Stencil:
                        {
                            QueueRenderSystemOp(finalState,
                                                new RSStencilOperation(pass.StencilCheck, pass.StencilFunc,
                                                                       pass.StencilRefValue,
                                                                       pass.StencilMask, pass.StencilFailOp,
                                                                       pass.StencilDepthFailOp,
                    ...
                    case CompositorPassType.RenderScene:
                        {
                            if (pass.FirstRenderQueue < finalState.CurrentQueueGroupId)
                            {
                                ...
                                LogManager.Instance.Write(
                                    "Warning in compilation of Compositor {0}: Attempt to render queue {1} before {2}.",
                    ...
                    case CompositorPassType.RenderQuad:
                        {
                            ...
                            if (srcmat == null)
                            {
                                ...
                                LogManager.Instance.Write(
                                    "Warning in compilation of Compositor {0}: No material defined for composition pass.",
                            ...
                            if (srcmat.SupportedTechniques.Count == 0)
                            {
                                ...
                                LogManager.Instance.Write(
                                    "Warning in compilation of Compositor {0}: material {1} has no supported techniques.",
                                    this.compositor.Name,
                                    srcmat.Name);
                                break;
                            }
                                    ...
                                    if (!string.IsNullOrEmpty(inp.Name))
                                    {
                                        if (x < targetPass.TextureUnitStatesCount)
                                        {
                                            targetPass.GetTextureUnitState(x).SetTextureName(
                                                GetTextureInstance(inp.Name, inp.MrtIndex).Name);
                                        }
                                        else
                                        {
                    ...
                    case CompositorPassType.RenderCustom:
                        {
                            CompositeRenderSystemOperation customOperation = CompositorManager.Instance.
                                CustomCompositionPasses[pass.CustomType].CreateOperation(
                                    this, pass);
        ...
        public void CompileTargetOperations(List<CompositeTargetOperation> compiledState)
        {
            ...
            // Texture targets
            foreach (CompositionTargetPass target in this.technique.TargetPasses)
            {
                CompositeTargetOperation ts =
                    new CompositeTargetOperation(GetTextureInstance(target.OutputName).GetBuffer().GetRenderTarget());
        ...
        {
            // Final target
            CompositionTargetPass tpass = this.technique.OutputTarget;
        ...
        public void CreateResources(bool forResizeOnly)
        {
            ...
            foreach (CompositionTechnique.TextureDefinition def in this.technique.TextureDefinitions)
            {
                ...
                {
                    Compositor parentComp = this.technique.Parent;
                ...
                if (def.PixelFormats.Count > 1)
                {
                    string mrtBaseName = "c" + resourceDummyCounter++ + "/" + def.Name + "/" +
                                         this.chain.Viewport.Target.Name;
                    ...
                    foreach (PixelFormat p in def.PixelFormats)
                    {
                        ...
                        if (def.Pooled)
                        {
                            ...
                            tex = CompositorManager.Instance.GetPooledTexture(texName, mrtLocalName, width, height, p,
                                                                              fsaa, fsaahint,
                        ...
                        {
                            tex = TextureManager.Instance.CreateManual(texName,
                                                                       ResourceGroupManager.InternalResourceGroupName,
                                                                       TextureType.TwoD, width, height, 0, p,
                                                                       TextureUsage.RenderTarget,
                                                                       null, hwGamma && !PixelUtil.IsFloatingPoint(p),
                                                                       fsaa, fsaahint);
                        }
                ...
                {
                    string texName = "c" + resourceDummyCounter++ + "/" + def.Name + "/" +
                                     this.chain.Viewport.Target.Name;
                    ...
                    {
                        // get / create pooled texture
                        tex = CompositorManager.Instance.GetPooledTexture(texName, def.Name, width, height,
                                                                          def.PixelFormats[0], fsaa,
                                                                          fsaahint,
                                                                          hwGamma &&
                    ...
                    else
                    {
                        tex = TextureManager.Instance.CreateManual(texName,
                                                                   ResourceGroupManager.InternalResourceGroupName,
                                                                   TextureType.TwoD, width, height, 0,
                                                                   def.PixelFormats[0],
                                                                   TextureUsage.RenderTarget, null,
                                                                   hwGamma &&
                                                                   !PixelUtil.IsFloatingPoint(def.PixelFormats[0]), fsaa,
                                                                   fsaahint);
                    }
        ...
        public void FreeResources(bool forResizeOnly, bool clearReservedTextures)
        {
            ...
            if (clearReservedTextures)
            {
                if (forResizeOnly)
                {
                    List<CompositionTechnique.TextureDefinition> toDelete =
                        new List<CompositionTechnique.TextureDefinition>();
                    foreach (CompositionTechnique.TextureDefinition def in this.reservedTextures.Keys)
                    {
                                                      ...
                                                      out string fsaaHint)
        {
            ...
            foreach (CompositionTargetPass tp in this.technique.TargetPasses)
            {
                if (tp.OutputName == texname)
                {
                    if (tp.InputMode == CompositorInputMode.Previous)
                    {
                        ...
                        renderingScene = true;
                        foreach (CompositorInstance inst in this.chain.Instances)
                        {
                            if (inst == this)
                            {
                                break;
            ...
            if (renderingScene)
            {
                ...
                RenderTarget target = this.chain.Viewport.Target;
    ...
    public class RSStencilOperation : CompositeRenderSystemOperation
    {
        ...
        public override void Execute(SceneManager sm, RenderSystem rs)
        {
            ...
            rs.SetStencilBufferParams(this.func, this.refValue, this.mask, this.stencilFailOp, this.depthFailOp,
                                      this.passOp,
