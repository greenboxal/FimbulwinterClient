...
//     <id value="$Id: HardwareBufferManagerBase.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
    public abstract class HardwareBufferManagerBase : DisposableObject
    {
        ...
        protected virtual void DestroyAllDeclarations()
        {
            lock (VertexDeclarationsMutex)
            {
                foreach (VertexDeclaration decl in this.vertexDeclarations)
                {
                    DestroyVertexDeclarationImpl(decl);
                }
        ...
        protected virtual void DestroyAllBindings()
        {
            lock (VertexBufferBindingsMutex)
            {
                foreach (VertexBufferBinding bind in this.vertexBufferBindings)
                {
        ...
        /// </remarks>
        [OgreVersion(1, 7, 2)]
        public virtual void RegisterVertexBufferSourceAndCopy(HardwareVertexBuffer sourceBuffer,
        ...
        /// <param name="licenseType"> The type of license required on this buffer - automatic release causes this class to release licenses every frame so that they can be reallocated anew. </param>
        /// <param name="licensee"> Reference back to the class requesting the copy, which must implement <see
        ///    cref="IHardwareBufferLicensee" /> in order to be notified when the license expires. </param>
            ...
            lock (VertexBuffersMutex)
            {
                lock (TempBuffersMutex)
                {
                    ...
                    this.tempVertexBufferLicenses.Add(vbuf,
                                                      new VertexBufferLicense(sourceBuffer, licenseType,
                                                                              expiredDelayFrameThreshold,
                                                                              vbuf,
                                                                              licensee));
    ...
    /// <see cref="HardwareBufferManager.AllocateVertexBufferCopy(HardwareVertexBuffer, BufferLicenseRelease, IHardwareBufferLicensee, bool)"/>
        ...
        /// </remarks>
        /// <param name="bufferCopy"> The buffer copy. The caller is expected to no longer use this reference, since another user may well begin to modify the contents of the buffer. </param>
        [OgreVersion(1, 7, 2)]
        public virtual void ReleaseVertexBufferCopy(HardwareVertexBuffer bufferCopy)
        {
            lock (TempBuffersMutex)
            {
                ...
                {
                    VertexBufferLicense vbl = this.tempVertexBufferLicenses[bufferCopy];
        ...
        public virtual void FreeUnusedBufferCopies()
        {
            lock (TempBuffersMutex)
            {
                ...
                for (int i = 1; i < this.freeTempVertexBufferMap.Count; ++i)
                {
                    HardwareVertexBuffer[] keys = new HardwareVertexBuffer[this.freeTempVertexBufferMap.Count];
                    this.freeTempVertexBufferMap.Keys.CopyTo(keys, 0);
                    HardwareVertexBuffer icur = this.freeTempVertexBufferMap[keys[i]];
        ...
        public virtual void ReleaseBufferCopies(bool forceFreeUnused)
        {
            lock (TempBuffersMutex)
            {
                int numUnused = this.freeTempVertexBufferMap.Count;
                int numUsed = this.tempVertexBufferLicenses.Count;

                // Erase the copies which are automatic licensed out
                for (int i = 1; i < this.tempVertexBufferLicenses.Count; ++i)
                {
                    HardwareVertexBuffer[] keys = new HardwareVertexBuffer[this.tempVertexBufferLicenses.Count];
                    this.tempVertexBufferLicenses.Keys.CopyTo(keys, 0);
                    VertexBufferLicense vbl = this.tempVertexBufferLicenses[keys[i]];
        ...
        public virtual void ForceReleaseBufferCopies(HardwareVertexBuffer sourceBuffer)
        {
            lock (TempBuffersMutex)
            {
                ...
                List<HardwareVertexBuffer> freeCopies = (from m in this.freeTempVertexBufferMap
                                                         where m.Key == sourceBuffer && m.Value.UseCount <= 1
                                                         select m.Key).ToList();

                foreach (HardwareVertexBuffer v in freeCopies)
                {
        ...
        [OgreVersion(1, 7, 2)]
        protected HardwareVertexBuffer MakeBufferCopy(HardwareVertexBuffer source, BufferUsage usage,
        ...
        ///  automatically.
        ///</remarks>
        ///<param name="vertexDeclaration"> The <see cref="VertexDeclaration" /> used for this buffer, this based on the kind of data you expect to populate this buffer with. NOTE Ogre just use an int parameter containing the vertex size, instead of the VertexDeclaration. DO NOT CHANGE IT, 'cause this parameter is in use in the Xna render system. </param>
        ///<param name="numVerts"> The number of vertices in this buffer. </param>
        ///<param name="usage"> One or more members of the BufferUsage enumeration; you are strongly advised to use StaticWriteOnly wherever possible, if you need to update regularly, consider WriteOnly and useShadowBuffer=true. </param>
        ///<param name="useShadowBuffer"> </param>
        [OgreVersion(1, 7, 2)]
#if NET_40
        public abstract HardwareVertexBuffer CreateVertexBuffer(VertexDeclaration vertexDeclaration, int numVerts,
                                                                BufferUsage usage, bool useShadowBuffer = false);
#else
		public abstract HardwareVertexBuffer CreateVertexBuffer( VertexDeclaration vertexDeclaration, int numVerts,
        ...
        /// </summary>
        /// <param name="type"> The type in index, either 16- or 32-bit, depending on how many vertices you need to be able to address. </param>
        ...
        ///    cref="BufferUsage.WriteOnly" /> for the main buffer. If you use this option, all reads and writes will be done to the shadow buffer, and the shadow buffer will be synchronized with the real buffer at an appropriate time. </param>
        [OgreVersion(1, 7, 2)]
#if NET_40
