...
namespace Axiom.Graphics
{
    internal static class CompositorScriptLoader
    {
        ...
        public static void ParseScript(CompositorManager compositorManager, Stream data, string groupName,
                                       string fileName)
        {
            string line = "";
                            ...
                            case CompositorScriptSection.Target:
                                switch (token)
                                {
                                    ...
                                    case "pass":
                                        ...
                                        context.seenOpen = false;
                                        if (!OptionCount(context, token, 1, args.Length) &&
                                            !OptionCount(context, token, 2, args.Length))
                                        {
                                            break;
                                        }
                                        arg = args[0].Trim();
                                        switch (arg)
                                        {
                                            ...
                                            default:
                                                LogError(context,
                                                         "In line '{0}', unrecognized compositor pass type '{1}'", arg);
                                                break;
                                        }
                                        break;
                                    case "}":
                                    ...
                                    case "buffers":
                                        ...
                                        foreach (string cb in args)
                                        {
                                            switch (cb)
                                            {
                                                ...
                                                default:
                                                    LogError(context,
                                                             "When parsing pass clear buffers options, illegal option '{0}'",
                                                             cb);
                                                    break;
                                            }
                                        }
        ...
        private static uint ParseUint(CompositorScriptContext context, string s)
        {
            ...
            {
                LogError(context, "Error converting string '{0}' to unsigned integer; error message is '{1}'", n,
                         e.Message);
                return 0;
            }
        }
