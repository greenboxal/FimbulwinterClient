...
//     <id value="$Id: EdgeListBuilder.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
    ...
    public class EdgeListBuilder : AnyBuilder
    {
...
#endif
            {
                ...
                for (int t = 0; t < iterations; t++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            CreateEdge(vertexSet, triStart + t, tri.vertIndex[0], tri.vertIndex[1],
                                       tri.sharedVertIndex[0],
                                       tri.sharedVertIndex[1]);
                        }
                        if (tri.sharedVertIndex[1] < tri.sharedVertIndex[2])
                        {
                            CreateEdge(vertexSet, triStart + t, tri.vertIndex[1], tri.vertIndex[2],
                                       tri.sharedVertIndex[1],
                                       tri.sharedVertIndex[2]);
                        }
                        if (tri.sharedVertIndex[2] < tri.sharedVertIndex[0])
                        {
                            CreateEdge(vertexSet, triStart + t, tri.vertIndex[2], tri.vertIndex[0],
                                       tri.sharedVertIndex[2],
                                       tri.sharedVertIndex[0]);
                        }
                    }
        ...
        protected void CreateEdge(int vertexSet, int triangleIndex, int vertexIndex0, int vertexIndex1,
                                  int sharedVertIndex0,
        ...
        public void DebugLog(Log log)
        {
            ...
            for (i = 0; i < indexDataList.Count; i += 3)
            {
                ...
                for (i = 0; i < this.vertices.Count; i++)
                {
                    ...
                    log.Write("Common vertex {0}: (vertexSet={1}, originalIndex={2}, position={3}", i, c.vertexSet,
                              c.index,
                              c.position);
                }
            }
