...
//     <id value="$Id: RenderSystem.cs 3315 2012-05-31 20:37:22Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
    ...
    public abstract class RenderSystem : DisposableObject
    {
        ...
        [OgreVersion(1, 7, 2790)] protected HardwareOcclusionQueryList hwOcclusionQueries =
            new HardwareOcclusionQueryList();

        /// <summary>
        ///   List of current render targets (i.e. a <see cref="RenderWindow" />, or a<see cref="RenderTexture" />) by priority
        /// </summary>
        [OgreVersion(1, 7, 2790)] protected RenderTargetPriorityMap prioritizedRenderTargets =
            new RenderTargetPriorityMap();
        ...
        /// </remarks>
        /// <note>Has NO effect on windowed mode render targets. Only affects fullscreen mode.</note>
        ...
        {
            get { return this.globalInstanceVertexBuffer; }
            set
            {
        ...
        {
            get { return this.globalInstanceVertexBufferVertexDeclaration; }
            set { this.globalInstanceVertexBufferVertexDeclaration = value; }
        }
        ...
        {
            get { return this.globalNumberOfInstances; }
            set { this.globalNumberOfInstances = value; }
        }
        ...
        {
            get { return this.currentCapabilities; }
        }
        ...
        public virtual Viewport Viewport
        {
            ...
            set { throw new NotImplementedException("Abstract call: Viewport set"); }
        }
        ...
        {
            get { return this.cullingMode; }
            set { throw new MethodAccessException("Abstract call"); }
        }
        ...
        {
            get { return MaterialManager.DefaultSchemeName; }
        }
        ...
        {
            get { return this.currentPassIterationCount; }
            set { this.currentPassIterationCount = value; }
        }
        ...
        {
            get { return this.invertVertexWinding; }
            set { this.invertVertexWinding = value; }
        }
        ...
        {
            add { eventListeners += value; }
            remove { eventListeners -= value; }
        }

        [AxiomHelper(0, 8)] private readonly
            Dictionary<Axiom.Math.Tuple<string, Action<NameValuePairList>>, Action<string, NameValuePairList>>
            _namedEvents =
        ...
        {
            Math.Tuple<string, Action<NameValuePairList>> key =
                new Axiom.Math.Tuple<string, Action<NameValuePairList>>(name, handler);
        ...
        {
            get { return this.wBuffer; }
            set { this.wBuffer = value; }
        }
        ...
        ///     <li>[Other thread]Call
        ///       <see cref="RegisterThread" />
        ///       , notify main thread &amp; continue</li>
        ///     <li>[Main thread]Wake up &amp; call
        ///       <see cref="PostExtraThreadsStarted" />
        ///     </li>
        ///   </ol>
        ...
        /// </remarks>
        /// <note>This method takes no parameters - it must be called from the thread being
        ///   registered and that context is enough.</note>
        [OgreVersion(1, 7, 2790)]
        ...
        /// </summary>
        /// <param name="capabilities"> capabilities has to be a subset of the real capabilities and the caller is responsible for deallocating capabilities. </param>
        [OgreVersion(1, 7, 2790)]
        public virtual void UseCustomRenderSystemCapabilities(RenderSystemCapabilities capabilities)
        {
            ...
            {
                throw new AxiomException(
                    "Custom render capabilities must be set before the RenderSystem is initialized.");
            }
        ...
        /// </summary>
        /// <remarks>
        ///   Calling this method can cause the rendering system to modify the ConfigOptions collection.
        /// </remarks>
        ...
        public virtual void InitRenderTargets()
        {
            ...
            foreach (KeyValuePair<string, RenderTarget> item in this.renderTargets)
            {
        ...
        /// </remarks>
        /// <param name="op"> A rendering operation instance, which contains details of the operation to be performed. </param>
        [OgreVersion(1, 7, 2790)]
        public virtual void Render(RenderOperation op)
        {
            int val = op.useIndices ? op.indexData.indexCount : op.vertexData.vertexCount;
        ...
        public virtual void SetTextureUnitSettings(int texUnit, TextureUnitState tl)
        {
            ...
            Texture tex = (Texture) TextureManager.Instance.GetByName(tl.TextureName);
            ...
            SetTextureUnitFiltering(texUnit, tl.GetTextureFiltering(FilterType.Min),
            ...
            UVWAddressing uvw = tl.TextureAddressingMode;
            SetTextureAddressingMode(texUnit, uvw);

            // Set the texture border color only if needed.
            if (uvw.U == TextureAddressing.Border || uvw.V == TextureAddressing.Border ||
            ...
            for (int i = 0; i < tl.NumEffects; i++)
            {
                TextureEffect effect = tl.GetEffect(i);
        ...
        public virtual void Shutdown()
        {
            ...
            foreach (HardwareOcclusionQuery i in this.hwOcclusionQueries)
            {
                i.SafeDispose();
            }
            ...
            foreach (KeyValuePair<string, RenderTarget> it in this.renderTargets)
            {
        ...
        public virtual void UpdateAllRenderTargets(bool swapBuffers)
        {
            ...
            foreach (KeyValuePair<RenderTargetPriority, List<RenderTarget>> targets in this.prioritizedRenderTargets)
            {
                foreach (RenderTarget target in targets.Value)
                {
        ...
        /// </summary>
        /// <param name="buffers"> Combination of one or more elements of <see cref="Graphics.RenderTarget.FrameBuffer" /> denoting which buffers are to be cleared. </param>
        ...
        /// <param name="miscParams"> A collection of addition rendersystem specific options. </param>
        [OgreVersion(1, 7, 2790)]
        ...
        /// </summary>
        /// <param name="renderWindowDescriptions"> Array of structures containing the descriptions of each render window. The structure's members are the same as the parameters of CreateRenderWindow: <see
        ///    cref="CreateRenderWindow" /> </param>
        ...
        {
            int fullscreenWindowsCount = 0;

            for (int nWindow = 0; nWindow < renderWindowDescriptions.Count; ++nWindow)
            {
                RenderWindowDescription curDesc = renderWindowDescriptions[nWindow];
                ...
                {
                    for (int nSecWindow = nWindow + 1; nSecWindow < renderWindowDescriptions.Count; ++nSecWindow)
                    {
        ...
        /// </summary>
        /// <Remarks>Surfaces can be bound and unbound at will. This fails if Capabilities.MultiRenderTargetsCount is smaller than 2.</Remarks>
        /// <returns> </returns>
        [OgreVersion(1, 7, 2790)]
        ...
        /// <remarks>
        ///   All subclasses should call this method from within thier own intialize methods.
        /// </remarks>
        [OgreVersion(1, 7, 2790)]
    ...
    /// <see cref="Axiom.Graphics.RenderSystem.MakeProjectionMatrix(Radian, Real, Real, Real, out Matrix4, bool)"/>
        ...
        /// </remarks>
        [OgreVersion(1, 7, 2790)]
        public abstract void MakeProjectionMatrix(Real left, Real right, Real bottom, Real top, Real nearPlane,
                                                  Real farPlane,
                                                  out Matrix4 dest,
#if NET_40
                                                  bool forGpuProgram = false);
    ...
    /// <see cref="Axiom.Graphics.RenderSystem.MakeProjectionMatrix(Real, Real, Real, Real, Real, Real, out Matrix4, bool)"/>
        ...
        {
            foreach (KeyValuePair<PoolId, DepthBufferVec> itmap in this.depthBufferPool)
            {
                List<DepthBuffer>.Enumerator itor = itmap.Value.GetEnumerator();
                while (itor.MoveNext())
                {
        ...
        {
            SetSeparateSceneBlending(sourceFactor, destFactor, sourceFactorAlpha, destFactorAlpha,
        ...
        /// </summary>
        /// <remarks>
        ///   <para> The stencil buffer is used to mask out pixels in the render target, allowing you to do effects like mirrors, cut-outs, stencil shadows and more. Each of your batches of rendering is likely to ignore the stencil buffer, update it with new values, or apply it to mask the output of the render. The stencil test is: <PRE>(Reference Value &amp; Mask) CompareFunction (Stencil Buffer Value &amp; Mask)</PRE> The result of this will cause one of 3 actions depending on whether the test fails, succeeds but with the depth buffer check still failing, or succeeds with the depth buffer check passing too. </para>
        ///   <para> Unlike other render states, stencilling is left for the application to turn on and off when it requires. This is because you are likely to want to change parameters between batches of arbitrary objects and control the ordering yourself. In order to batch things this way, you'll want to use OGRE's separate render queue groups (see RenderQueue) and register a RenderQueueListener to get notifications between batches. </para>
        ///   <para> There are individual state change methods for each of the parameters set using this method. Note that the default values in this method represent the defaults at system start up too. </para>
        /// </remarks>
        [OgreVersion(1, 7, 2790)]
        ...
        {
            SetStencilBufferParams(function, refValue, -1, StencilOperation.Keep, StencilOperation.Keep,
        ...
        /// </summary>
        /// <param name="ambient"> The amount of ambient (sourceless and directionless) light an object reflects. Affected by the color/amount of ambient light in the scene. </param>
        /// <param name="diffuse"> The amount of light from directed sources that is reflected (affected by color/amount of point, directed and spot light sources) </param>
        /// <param name="specular"> The amount of specular light reflected. This is also affected by directed light sources but represents the color at the highlights of the object. </param>
        /// <param name="emissive"> The color of light emitted from the object. Note that this will make an object seem brighter and not dependent on lights in the scene, but it will not act as a light, so will not illuminate other objects. Use a light attached to the same SceneNode as the object for this purpose. </param>
        /// <param name="shininess"> A value which only has an effect on specular highlights (so specular must be non-black). The higher this value, the smaller and crisper the specular highlights will be, imitating a more highly polished surface. This value is not constrained to 0.0-1.0, in fact it is likely to be more (10.0 gives a modest sheen to an object). </param>
        [OgreVersion(1, 7, 2790)]
        public void SetSurfaceParams(ColorEx ambient, ColorEx diffuse, ColorEx specular, ColorEx emissive,
                                     Real shininess)
        {
        ...
        /// <param name="emissive"> The color of light emitted from the object. Note that this will make an object seem brighter and not dependent on lights in the scene, but it will not act as a light, so will not illuminate other objects. Use a light attached to the same SceneNode as the object for this purpose. </param>
        /// <param name="shininess"> A value which only has an effect on specular highlights (so specular must be non-black). The higher this value, the smaller and crisper the specular highlights will be, imitating a more highly polished surface. This value is not constrained to 0.0-1.0, in fact it is likely to be more (10.0 gives a modest sheen to an object). </param>
        /// <param name="tracking"> A bit field that describes which of the ambient, diffuse, specular and emissive colors follow the vertex color of the primitive. When a bit in this field is set its colorValue is ignored. This is a combination of TVC_AMBIENT, TVC_DIFFUSE, TVC_SPECULAR(note that the shininess value is still taken from shininess) and TVC_EMISSIVE. TVC_NONE means that there will be no material property tracking the vertex colors. </param>
        [OgreVersion(1, 7, 2790)]
        ...
        /// </summary>
        /// <param name="unit"> The index of the texture unit to modify. Multitexturing hardware can support multiple units (see TextureUnitCount) </param>
        /// <param name="enabled"> Boolean to turn the unit on/off </param>
        /// <param name="textureName"> The name of the texture to use - this should have already been loaded with TextureManager.Load. </param>
        [OgreVersion(1, 7, 2790)]
        ...
        /// </summary>
        /// <param name="unit"> The index of the texture unit to modify. Multitexturing hardware can support multiple units <see
        ///    cref="RenderSystemCapabilities.TextureUnitCount" /> </param>
        ...
        /// <param name="mipFilter"> The filter used between mipmap levels, <see cref="FilterOptions.None" /> disables mipmapping. </param>
        [OgreVersion(1, 7, 2790)]
