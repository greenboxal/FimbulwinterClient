...
//     <id value="$Id: Compositor.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Graphics
{
    ...
    public class Compositor : Resource
    {
        ...
        {
            get { return this.readOnlyTechniques; }
        }
        ...
        {
            get { return this.readOnlySupportedTechniques; }
        }
        ...
        protected override void dispose(bool disposeManagedResources)
        {
            if (!IsDisposed)
            {
                if (disposeManagedResources)
                {
                    ...
                    foreach (KeyValuePair<string, MultiRenderTarget> item in this.globalMRTs)
                    {
                        item.Value.Dispose();
                    }
                    this.globalMRTs.Clear();

                    foreach (KeyValuePair<string, Texture> item in this.globalTextures)
                    {
                        item.Value.Dispose();
                    }
                    return;
                ...
                if (def.Scope == CompositionTechnique.TextureScope.Global)
                {
                    ...
                    if (def.PixelFormats.Count > 1)
                    {
                        ...
                        foreach (PixelFormat p in def.PixelFormats)
                        {
                            ...
                            Texture tex = TextureManager.Instance.CreateManual(texName,
                                                                               ResourceGroupManager.
                                                                                   InternalResourceGroupName,
                                                                               TextureType.TwoD, def.Width, def.Height,
                                                                               0, 0, p,
                                                                               TextureUsage.RenderTarget, null,
                                                                               def.HwGammaWrite &&
                                                                               !PixelUtil.IsFloatingPoint(p),
                                                                               def.Fsaa ? 1 : 0);
                    ...
                    else
                    {
                        ...
                        Texture tex = TextureManager.Instance.CreateManual(texName,
                                                                           ResourceGroupManager.
                                                                               InternalResourceGroupName,
                                                                           TextureType.TwoD, def.Width, def.Height, 0,
                                                                           def.PixelFormats[0],
                                                                           TextureUsage.RenderTarget, null,
                                                                           def.HwGammaWrite &&
                                                                           !PixelUtil.IsFloatingPoint(
                                                                               def.PixelFormats[0]),
                                                                           def.Fsaa ? 1 : 0);
