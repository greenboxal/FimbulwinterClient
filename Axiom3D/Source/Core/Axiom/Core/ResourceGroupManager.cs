...
//     <id value="$Id: ResourceGroupManager.cs 3316 2012-05-31 20:50:21Z borrillis $"/>
// </file>
...
namespace Axiom.Core
{
    ...
    public class ResourceGroupManager : DisposableObject, ISingleton<ResourceGroupManager>
    {
        ...
        {
            get { return this._resourceManagers; }
        }
        ...
        {
            get { return this._scriptLoaderOrders; }
        }
        ...
        {
            get { return this._resourceGroupListeners; }
        }
        ...
        ///  <para> Once you have defined a resource group, resources which will be loaded as part of it are defined in one of 3 ways: <ol>
        ...
        ///                                                                                                                            </ol> You must remember to call initialiseResourceGroup if you intend to use the first 2 types. </para>
        ///</remarks>
        ...
        ///  <para> Failure to call this method means that <see cref="LoadResourceGroup(string)" /> will do nothing, and any resources you define in scripts will not be found. Similarly, once you have called this method you won't be able to pick up any new scripts or pre-declared resources, unless you call <see
        ///   cref="ClearResourceGroup" /> , set up declared resources, and call this method again. </para>
        ///  <para> When you call <see cref="Root.Initialize(bool)" /> , all resource groups that have already been created are automatically initialised too. Therefore you do not need to call this method for groups you define and set up before you call <see
        ///   cref="Root.Initialize(bool)" /> . However, since one of the most useful features of resource groups is to set them up after the main system initialization has occurred (e.g. a group per game level), you must remember to call this method for the groups you create after this. </para>
        ///</remarks>
...
#warning TODO
        ...
        public void AddResourceLocation(string name, string locType, string resGroup, bool recursive, bool monitor)
        {
            ...
            LogManager.Instance.Write("Added resource location '{0}' of type '{1}' to resource group '{2}'{3}", name,
                                      locType,
        ...
        ///     <para> Declared resources are not created as Resource instances (and thus are not available through their ResourceManager) until <see
        ///    cref="InitializeResourceGroup" /> is called, at which point all declared resources will become created (but unloaded) Resource instances, along with any resources declared in scripts in resource locations associated with the group. </para>
        ///   </remarks>
		...
		*/
                                              ...
                                              Resource resourceBeingLoaded)
        {
            ...
            if (grp.ResourceIndexCaseSensitive.ContainsKey(resourceName))
            {
                ...
                if (stream == null)
                {
                    FileInfoList fileList = FindResourceFileInfo(groupName,
                                                                 "*" + System.IO.Path.GetExtension(resourceName));
            ...
            if (searchGroupsIfNotFound)
            {
                ...
                {
                    throw new IO.FileNotFoundException("Cannot locate resource " + resourceName + " in resource group " +
                                                       groupName +
                                                       " or any other group.");
                }
            }
        ...
        {
            foreach (KeyValuePair<string, ResourceManager> pair in this._resourceManagers)
            {
        ...
        public List<string> GetResourceGroups()
        {
            ...
            foreach (KeyValuePair<string, ResourceGroup> pair in this._resourceGroups)
            {
        ...
        public void notifyAllResourcesRemoved(ResourceManager manager)
        {
            ...
            foreach (KeyValuePair<string, ResourceGroup> rgPair in this._resourceGroups)
            {
        ...
        private void _parseResourceGroupScripts(ResourceGroup grp)
        {
            ...
            List<Math.Tuple<IScriptLoader, List<FileInfoList>>> scriptLoaderFileList =
                new List<Axiom.Math.Tuple<IScriptLoader, List<FileInfoList>>>();

            int scriptCount = 0;
            // Iterate over script users in loading order and get streams
            foreach (KeyValuePair<float, List<IScriptLoader>> pairsl in this._scriptLoaderOrders)
            {
            ...
            foreach (Math.Tuple<IScriptLoader, List<FileInfoList>> slfli in scriptLoaderFileList)
            {
