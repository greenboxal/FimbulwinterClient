...
namespace Axiom.Core
{
    public partial class StaticGeometry
    {
        ...
        public class GeometryBucket : DisposableObject, IRenderable
        {
            ...
            {
                get { return this.parent.Material; }
            }

            public Technique Technique
            {
                get { return this.parent.CurrentTechnique; }
            }
            ...
            {
                get { return this.parent.Parent.Parent.Center; }
            }

            public LightList Lights
            {
                get { return this.parent.Parent.Parent.Lights; }
            }

            public bool CastsShadows
            {
                get { return this.parent.Parent.Parent.CastShadows; }
            }
            ...
            public GeometryBucket(MaterialBucket parent, string formatString, VertexData vData, IndexData iData)
            {
                ...
                VertexElement blendIndices =
                    this.vertexData.vertexDeclaration.FindElementBySemantic(VertexElementSemantic.BlendIndices);
                VertexElement blendWeights =
                    this.vertexData.vertexDeclaration.FindElementBySemantic(VertexElementSemantic.BlendWeights);
                if (blendIndices != null && blendWeights != null)
                {
                    ...
                    Debug.Assert(
                        ...
                        this.vertexData.vertexBufferBinding.GetBuffer(source).VertexSize,
            ...
            public void Build(bool stencilShadows, int logLevel)
            {
                ...
                if (logLevel <= 1)
                {
                    LogManager.Instance.Write(
                        "GeometryBucket.Build: Creating index buffer indexType {0} indexData.indexCount {1}",
...
#endif
                {
                    BufferBase[] destBufferPtrs = new BufferBase[binds.BindingCount];
                    for (b = 0; b < binds.BindingCount; ++b)
                    {
                        ...
                        {
                            LogManager.Instance.Write(
                                "GeometryBucket.Build b {0}, binds.BindingCount {1}, vertexCount {2}, dcl.GetVertexSize(b) {3}",
                                b,
                        ...
                        HardwareVertexBuffer vbuf = HardwareBufferManager.Instance.CreateVertexBuffer(dcl.Clone(b),
                                                                                                      vertexCount,
                                                                                                      BufferUsage.
                                                                                                          StaticWriteOnly);
                    ...
                    foreach (QueuedGeometry geom in this.queuedGeometry)
                    {
                        ...
                        for (b = 0; b < binds.BindingCount; ++b)
                        {
                            ...
                            destBufferPtrs[b].Ptr = CopyVertices(srcBinds.GetBuffer(b), destBufferPtrs[b],
                                                                 bufferElements[b], geom,
                                                                 regionCenter);
                        }
            ...
            {
                get { return this.parent.Parent.Parent.NumWorldTransforms; }
            }
