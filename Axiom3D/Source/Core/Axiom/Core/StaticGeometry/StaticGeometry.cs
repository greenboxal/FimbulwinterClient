...
namespace Axiom.Core
{
    ...
    ///       use many different materials; try to keep the number down.</li>
    ///     l</ul>
    ///   <br />
    ...
    public partial class StaticGeometry
    {
        ...
        {
            get { return this.squaredUpperDistance; }
        }
        ...
        {
            get { return this.regionDimensions; }
            set
            {
        ...
        {
            Vector3 min = new Vector3(((float) x - regionHalfRange)*this.regionDimensions.x + this.origin.x,
        ...
        protected Vector3 GetRegionCenter(ushort x, ushort y, ushort z)
        {
            return
                new Vector3(
                    ((float) x - regionHalfRange)*this.regionDimensions.x + this.origin.x + this.halfRegionDimensions.x,
        ...
        protected void GetRegionIndexes(Vector3 point, out ushort x, out ushort y, out ushort z)
        {
            ...
            // Check bounds
            if (ix < regionMinIndex || ix > regionMaxIndex || iy < regionMinIndex || iy > regionMaxIndex ||
                iz < regionMinIndex ||
                iz > regionMaxIndex)
            {
                                                 ...
                                                 Vector3 scale)
        {
...
#endif
            {
                ...
                for (int j = 0; j < vertexData.vertexCount; ++j, vertex += vbuf.VertexSize)
                {
                    ...
                    Vector3 pt = new Vector3(pFloat[0], pFloat[1], pFloat[2]);
...
#endif
            {
                ...
                for (short b = 0; b < numvbufs; ++b)
                {
                    ...
                    HardwareVertexBuffer newBuf =
                        HardwareBufferManager.Instance.CreateVertexBuffer(oldBuf.VertexDeclaration, indexRemap.Count,
                                                                          BufferUsage.Static);
                    // rebind
                ...
                HardwareIndexBuffer ibuf = HardwareBufferManager.Instance.CreateIndexBuffer(id.indexBuffer.Type,
                                                                                            id.indexCount,
                                                                                            BufferUsage.Static);
        ...
        /// </remarks>
        /// <note>The SceneNode you pass in will not be automatically detached from
        ...
        ///   of rendering
        ///   <i>both</i>
        ///   the original objects and their new static
        ///   versions! We don't do this for you incase you are preparing this
        ///   in advance and so don't want the originals detached yet.</note>
        ...
        /// </remarks>
        /// <note>Once you have called this method, you can no longer add any more
        ///   entities.</note>
        public void Build()
        {
            if (this.logLevel <= 1)
