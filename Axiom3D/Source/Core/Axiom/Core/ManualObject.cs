...
//     <id value="$Id: ManualObject.cs 3315 2012-05-31 20:37:22Z borrillis $"/>
// </file>
    ...
    public class ManualObject : MovableObject
    {
        ...
        protected virtual void ResizeTempVertexBufferIfNeeded(int numVerts)
        {
            ...
            if (newSize > this.tempVertexSize || this.tempVertexBuffer == null)
            {
                ...
                else
                {
                    ...
                    newSize = (int) Utility.Max(newSize, this.tempVertexSize*2.0f);
                }
                // copy old data
        ...
        protected virtual void ResizeTempIndexBufferIfNeeded(int numInds)
        {
            ...
            if (newSize > this.tempIndexSize || this.tempIndexBuffer == null)
            {
                ...
                {
                    // increase to at least double current
                    newSize = (int) Utility.Max(newSize, (float) this.tempIndexSize*2);
                }
                numInds = newSize/sizeof (UInt16);
                ushort[] tmp = this.tempIndexBuffer;
        ...
        protected virtual void CopyTempVertexToBuffer()
        {
            ...
            {
                // get base pointer
                BufferBase buf = BufferBase.Wrap(this.tempVertexBuffer);
                buf.Ptr = this.declSize*(rop.vertexData.vertexCount - 1);

                float* pFloat = buf.ToFloatPointer();
                uint* pRGBA = buf.ToUIntPointer();
        ...
        {
            get { return this.sectionList.Count; }
        }
        ...
        {
            ResetTempAreas();
            foreach (ManualObjectSection item in this.sectionList)
            {
                item.Dispose();
            }
            ...
            this.edgeList = null;
            this.anyIndexed = false;
            foreach (ShadowRenderable item in this.shadowRenderables)
            {
                item.Dispose();
            }
		...
		*/
        ...
        public virtual ManualObjectSection End()
        {
            ...
            else // not an empty section
            {
                ...
                if (vbufNeedsCreating)
                {
                    ...
                    int vertexCount = Utility.Max(rop.vertexData.vertexCount, this.estVertexCount);

                    vbuf = HardwareBufferManager.Instance.CreateVertexBuffer(rop.vertexData.vertexDeclaration,
                                                                             vertexCount,
                                                                             this.dynamic
                ...
                if (ibufNeedsCreating)
                {
                    ...
                    // to allow for user-configured growth area
                    int indexCount = Utility.Max(rop.indexData.indexCount, this.estIndexCount);
                    rop.indexData.indexBuffer = HardwareBufferManager.Instance.CreateIndexBuffer(IndexType.Size16,
                                                                                                 indexCount,
                                                                                                 this.dynamic
                                                                                                     ? BufferUsage.
                                                                                                           DynamicWriteOnly
        ...
        {
            get { return (AxisAlignedBox) this.AABB.Clone(); }
        }
                                                                        ...
                                                                        float extrusionDistance, int flags)
        {
            ...
            for (int i = 0; i < this.shadowRenderables.Capacity; i++)
            {
                ...
                if (init)
                {
                    ...
                    esr = new ManualObjectSectionShadowRenderable(this, indexBuffer, egi.vertexData,
        ...
        public class ManualObjectSectionShadowRenderable : ShadowRenderable
        {
            ...
            public ManualObjectSectionShadowRenderable(ManualObject parent, HardwareIndexBuffer indexBuffer,
                                                       VertexData vertexData, bool createSeparateLightCap,
                                                       bool isLightCap)
            {
                this.parent = parent;
