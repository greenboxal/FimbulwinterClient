...
//     <id value="$Id: Camera.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Core
{
    ...
    public class Camera : Frustum
    {
        ...
        public Vector3 Direction
        {
            ...
            get { return this.orientation*-Vector3.UnitZ; }
            set
            {
                ...
                if (this.isYawFixed)
                {
                    Vector3 xVector = this.yawFixedAxis.Cross(zAdjustVector);
                    xVector.Normalize();
        ...
        public Real LodBias
        {
            get { return this.sceneLodFactor; }
            set
            {
        ...
        public Camera LodCamera
        {
            ...
            set { this._lodCamera = this == value ? null : value; }
        }
        ...
        {
            get { return this.invSceneLodFactor; }
        }
        ...
        {
            get { return this.useRenderingDistance; }
            set { this.useRenderingDistance = value; }
        }
            ...
            get
            {
                ...
                if (_recalculateView)
                {
                    if (IsReflected)
                    {
                        ...
                        Vector3 dir = this.realOrientation*Vector3.NegativeUnitZ;
                        Vector3 rdir = dir.Reflect(ReflectionPlane.Normal);
                        Vector3 up = this.realOrientation*Vector3.UnitY;
        ...
        public new bool IsObjectVisible(AxisAlignedBox box)
        {
            if (null != CullFrustum)
            {
        ...
        public new bool IsObjectVisible(Vector3 vertex)
        {
            if (null != CullFrustum)
            {
        ...
        public new Vector3[] WorldSpaceCorners
        {
            get { return null != CullFrustum ? CullFrustum.WorldSpaceCorners : base.WorldSpaceCorners; }
        }

        public new Plane[] FrustumPlanes
        {
            get
            {
                UpdateFrustumPlanes();
        ...
        [OgreVersion(1, 7, 2790)]
        public override bool ProjectSphere(Sphere sphere, out float left, out float top, out float right,
                                           out float bottom)
        {
            if (null != CullFrustum)
            {
        ...
        public void MoveRelative(Vector3 offset)
        {
            ...
            Vector3 transform = this.orientation*offset;
        ...
        {
            Vector3 xAxis = this.orientation*Vector3.UnitX;
        ...
        public void Roll(float degrees)
        {
            ...
            Vector3 zAxis = this.orientation*Vector3.UnitZ;
        ...
        ///</remarks>
        ///<param name="enabled"> If true, the Camera will track the SceneNode supplied as the next parameter (cannot be null). If false the camera will cease tracking and will remain in it's current orientation. </param>
        ///<param name="target"> The SceneNode which this Camera will track. Make sure you don't delete this SceneNode before turning off tracking (e.g. SceneManager.ClearScene will delete it so be careful of this). Can be null if and only if the enabled param is false. </param>
        [AxiomHelper(0, 8, "default overload")]
                                                                      ...
                                                                      Real planeOffset)
        {
            ...
            int[] infpt = new[]
                              {
                                  0, 0, 0, 0
                              }; // 0=finite, 1=infinite, 2=straddles infinity
            ...
            IEnumerator<Vector3> edir = dir.GetEnumerator();
            for (int i = 0; i < 4; i++)
            {
                edir.MoveNext();
                Vector3 cur = edir.Current;
                Real test = cur.z*delta;
                if (test == 0.0)
                {
                    vec[i] = cur;
                    infpt[i] = 1;
        ...
        public virtual void ForwardIntersect(Plane worldPlane, IList<Vector4> intersect3D)
        {
            ...
            Vector3 trCorner = WorldSpaceCorners[0];
            Vector3 tlCorner = WorldSpaceCorners[1];
            Vector3 blCorner = WorldSpaceCorners[2];
            Vector3 brCorner = WorldSpaceCorners[3];
            ...
            Quaternion invPlaneRot = pval.Normal.GetRotationTo(Vector3.UnitZ);

            Vector3[] vec = new Vector3[4];

            // get rotated light
            Vector3 lPos = invPlaneRot*DerivedPosition;
            ...
            IEnumerable<Vector4> iPnt = GetRayForwardIntersect(lPos, vec, -pval.D);
            ...
            //if (intersect3D != null) // cant be null
            {
                ...
                intersect3D.Clear();
                foreach (Vector4 v in iPnt)
                {
                    Vector3 intersection = planeRot*new Vector3(v.x, v.y, v.z);
        ...
        public Plane GetWindowPlane(int index)
        {
            ...
            return this.windowClipPlanes[index];
        }

        /// <summary>
        ///   Gets a world space ray as cast from the camera through a viewport position.
        /// </summary>
        /// <param name="screenX"> The x position at which the ray should intersect the viewport, in normalized screen coordinates [0,1]. </param>
        /// <param name="screenY"> The y position at which the ray should intersect the viewport, in normalized screen coordinates [0,1]. </param>
        /// <param name="ray"> Ray instance to populate with result </param>
        /// <returns> </returns>
                                                 ...
                                                 PlaneBoundedVolume outVolume, bool includeFarPlane)
        {
            ...
            if (ProjectionType == Projection.Perspective)
            {
                ...
                // top plane
                Vector3 normal = ul.Direction.Cross(ur.Direction);
                normal.Normalize();
        ...
        {
            Vector3 dir = this.orientation*new Vector3(0, 0, -1);
