...
//     <id value="$Id: SceneManager.cs 3303 2012-05-31 12:10:26Z borrillis $"/>
// </file>
...
namespace Axiom.Core
{
    ...
    public abstract class SceneManager : DisposableObject
    {
        ...
        {
            get { return this.specialCaseRenderQueueList; }
        }
        ...
        {
            get { return this.worldGeometryRenderQueueId; }

            set { this.worldGeometryRenderQueueId = value; }
        }
        ...
        /// <summary>
        ///   A list of lights in the scene for easy lookup.
        /// </summary>
        public ICollection<MovableObject> Lights
        {
            get { return GetMovableObjectCollection(LightFactory.TypeName).Values; }
        }
        ...
        {
            add { this._queueStartedEvent.EventSinks += value; }
            remove { this._queueStartedEvent.EventSinks -= value; }
        }

        private readonly ChainedEvent<EndRenderQueueEventArgs> _queueEndedEvent =
            new ChainedEvent<EndRenderQueueEventArgs>();
        ...
        {
            add { this._queueEndedEvent.EventSinks += value; }
            remove { this._queueEndedEvent.EventSinks -= value; }
        }

        /// <summary>
        ///   Will fire before FindVisibleObjects is called
        /// </summary>
        public event FindVisibleObjectsEvent PreFindVisibleObjects;

        /// <summary>
        ///   Will fire after FindVisibleObjects is called
        /// </summary>
        ...
        {
            get { return this.autoParamDataSource; }
        }
        ...
        public virtual Animation CreateAnimation(string name, float length)
        {
            ...
            {
                throw new AxiomException(string.Format("An animation with the name '{0}' already exists in the scene.",
                                                       name));
            }
        ...
        ///</summary>
        ///<remarks>
        ///  <para> You can create Animation objects for animating SceneNode obejcts using the CreateAnimation method. However, in order to actually apply those animations you have to call methods on Node and Animation in a particular order (namely Node.ResetToInitialState and Animation.Apply). To make this easier and to help track the current time position of animations, the AnimationState object is provided. </para>
        ///  <para> So if you don't want to control animation application manually, call this method, update the returned object as you like every frame and let SceneManager apply the animation state for you. </para>
        ///  <para> Remember, AnimationState objects are disabled by default at creation time. Turn them on when you want them using their Enabled property. </para>
        ///  <para> Note that any SceneNode affected by this automatic animation will have it's state reset to it's initial position before application of the animation. Unless specifically modified using Node.SetInitialState the Node assumes it's initial state is at the origin. If you want the base state of the SceneNode to be elsewhere, make your changes to the node using the standard transform methods, then call SetInitialState to 'bake' this reference position into the node. </para>
        ///</remarks>
        ...
        /// <param name="caption"> The text tyo display </param>
        /// <param name="fontName"> The font to use for the text, must be already loaded as a resource. </param>
        ...
        /// <returns> An instance of MovablText. </returns>
        /// <exception cref="AxiomException">Thrown if the names does not exists in the collection.</exception>
        ...
        /// </summary>
        /// <param name="name"> The name of the object to retrieve. </param>
        /// <returns> An instance of ManualObject. </returns>
        /// <exception cref="AxiomException">Thrown if the names does not exists in the collection.</exception>
        ...
        ///</summary>
        ///<remarks>
        ///  <p>Overlays can be used to render heads-up-displays (HUDs), menu systems,
        ///    cockpits and any other 2D or 3D object you need to appear above the
        ///    rest of the scene. See the Overlay class for more information.</p>
        ///  <p>NOTE: after creation, the Overlay is initially hidden. You can create
        ///    as many overlays as you like ready to be displayed whenever. Just call
        ///    Overlay.Show to display the overlay.</p>
        ///</remarks>
        ...
        {
            Overlay newOverlay = OverlayManager.Instance.Create(name);
        ...
        ///</summary>
        ///<returns> The render queue in use by this scene manager. Note: The queue is created if it doesn't already exist. </returns>
        ...
        public virtual void ManualRender(RenderOperation op, Pass pass, Viewport vp, Matrix4 worldMatrix,
                                         Matrix4 viewMatrix,
        ...
        protected virtual IList FindShadowCastersForLight(Light light, Camera camera)
        {
            ...
            else
            {
                ...
                if (camera.IsObjectVisible(s))
                {
                    ...
                    this.shadowCasterQueryListener.Prepare(lightInFrustum, volumeList, light, camera,
                                                           this.shadowCasterList,
                                                           light.ShadowFarDistanceSquared);
        ...
        private void InitShadowStencilPass()
        {
            ...
            if (matStencil == null)
            {
                ...
                if (this.targetRenderSystem.Capabilities.HasCapability(Capabilities.VertexPrograms))
                {
                    ...
                    this.finiteExtrusionParams.SetAutoConstant(0,
                                                               GpuProgramParameters.AutoConstantType.WorldViewProjMatrix);
                    this.finiteExtrusionParams.SetAutoConstant(4,
                                                               GpuProgramParameters.AutoConstantType.
                                                                   LightPositionObjectSpace);
                    this.finiteExtrusionParams.SetAutoConstant(5,
                                                               GpuProgramParameters.AutoConstantType.
                                                                   ShadowExtrusionDistance);
                }
                matStencil.Compile();
        ...
        protected virtual Pass DeriveShadowCasterPass(Pass pass)
        {
            if (IsShadowTechniqueTextureBased)
            {
                    ...
                    pass.AlphaRejectFunction != CompareFunction.AlwaysPass)
                {
                    ...
                    int origPassTUCount = pass.TextureUnitStatesCount;
                    for (int t = 0; t < origPassTUCount; ++t)
                    {
                        ...
                        tex.SetColorOperationEx(LayerBlendOperationEx.Source1, LayerBlendSource.Manual,
        ...
        protected virtual Pass DeriveShadowReceiverPass(Pass pass)
        {
            if (IsShadowTechniqueTextureBased)
            {
                ...
                if (IsShadowTechniqueAdditive)
                {
                    ...
                    {
                        int targetIndex = t + 1;
                        TextureUnitState tex = (retPass.TextureUnitStatesCount <= targetIndex
        ...
        protected virtual void RenderShadowVolumesToStencil(Light light, Camera camera)
        {
            ...
            CheckShadowCasters(casters, nearClipVol, light, extrudeInSoftware, finiteExtrude, zfailAlgo, camera,
                               extrudeDistance,
        ...
        private void EnableHardwareShadowExtrusion(Light light, bool finiteExtrude)
        {
            ...
            this.shadowStencilPass.SetVertexProgram(ShadowVolumeExtrudeProgram.GetProgramName(light.Type, finiteExtrude,
                                                                                              false));
            ...
            {
                this.shadowDebugPass.SetVertexProgram(ShadowVolumeExtrudeProgram.GetProgramName(light.Type,
                                                                                                finiteExtrude, true));
        ...
        private void CheckShadowCasters(IList casters, PlaneBoundedVolume nearClipVol, Light light,
                                        ...
                                        bool stencil2sided, LightList tmpLightList)
        {
            ...
            for (int ci = 0; ci < casters.Count; ci++)
            {
                ...
                if (
                    !((flags & (int) ShadowRenderableFlags.ExtrudeToInfinity) != 0 &&
                ...
                IEnumerator renderables = caster.GetShadowVolumeRenderableEnumerator(this.shadowTechnique, light,
                                                                                     this.shadowIndexBuffer,
                                                                                     extrudeInSoftware, extrudeDistance,
                                                                                     flags);
        ...
        ///<returns> A Pass object that was used instead of the one passed in, can happen when rendering shadow passes </returns>
        public virtual Pass SetPass(Pass pass, bool evenIfSuppressed, bool shadowDerivation)
        {
            ...
            if (!this.suppressRenderStateChanges || evenIfSuppressed)
            {
                ...
                if (passSurfaceAndLightParams)
                {
                    ...
                    {
                        this.targetRenderSystem.SetSurfaceParams(pass.Ambient, pass.Diffuse, pass.Specular,
                    ...
                    else
                    {
                        ...
                        this.targetRenderSystem.SetSurfaceParams(ColorEx.White, ColorEx.Black, ColorEx.Black,
                                                                 ColorEx.Black, 0,
                ...
                if (pass.HasSeparateSceneBlending)
                {
                    this.targetRenderSystem.SetSeparateSceneBlending(pass.SourceBlendFactor, pass.DestinationBlendFactor,
                                                                     pass.SourceBlendFactorAlpha,
                                                                     pass.DestinationBlendFactorAlpha,
                ...
                else
                {
                    ...
                    {
                        this.targetRenderSystem.SetSeparateSceneBlending(pass.SourceBlendFactor,
                                                                         pass.DestinationBlendFactor,
                                                                         pass.SourceBlendFactor,
                                                                         pass.DestinationBlendFactor,
                                                                         pass.SceneBlendingOperation,
                                                                         pass.SceneBlendingOperationAlpha);
                    }
        ...
        protected Mesh CreateSkyboxPlane(BoxPlane plane, float distance, Quaternion orientation, string groupName)
        {
            ...
            Mesh planeModel = modelMgr[meshName];
                                          ...
                                          Quaternion orientation, string groupName)
        {
            ...
            Mesh planeMesh = meshManager[meshName];
            ...
            float planeSize = distance*2;
            int segments = 16;
            planeMesh = meshManager.CreateCurvedIllusionPlane(meshName, groupName, p, planeSize, planeSize, curvature,
                                                              segments,
        ...
        ///</remarks>
        ///<param name="mode"> Set up the mode of fog as described in the FogMode enum, or set to FogMode.None to turn off. </param>
        ///<param name="color"> The color of the fog. Either set this to the same as your viewport background color, or to blend in with a skydome or skybox. </param>
        ///<param name="density"> The density of the fog in Exp or Exp2. mode, as a value between 0 and 1. The default is 0.001. </param>
        ///<param name="linearStart"> Distance in world units at which linear fog starts to encroach. Only applicable if mode is </param>
        ///<param name="linearEnd"> Distance in world units at which linear fog becomes completely opaque. Only applicable if mode is </param>
        ...
        ///<param name="orientation"> Specifies the orientation of the box. By default the 'top' of the box is deemed to be in the +y direction, and the 'front' at the -z direction. You can use this parameter to rotate the sky if you want. </param>
        ///<param name="groupName"> </param>
        ...
        public virtual void SetShadowTextureSettings(ushort size, ushort count, PixelFormat format)
        {
            if (this.shadowTextures.Count > 0 &&
                (count != this.shadowTextureCount || size != this.shadowTextureSize ||
        ...
        /// </summary>
        /// <note>CompositorChain does this automatically, no need to call manually.</note>
        public CompositorChain ActiveCompositorChain
        {
            get { return this._activeCompositorChain; }
            set { this._activeCompositorChain = value; }
        }
        ...
        {
            get { return this._defaultShadowCameraSetup; }
            set { this._defaultShadowCameraSetup = value; }
        }
        ...
        ///  <ul>
        ///    <li>Stencil-based approaches can be used to
        ...
        ///</summary>
        ///<remarks>
        ///  <p>Axiom comes configured with a set of defaults for newly created
        ...
        ///    with the new defaults you have specified.</p>
        ///  <p>The default settings begin as a single Technique with a single, non-programmable Pass:
        ///    <ul>
        ...
        {
            get { return this._lightsDirtyCounter; }
        }
        ...
        protected internal void RenderScene(Camera camera, Viewport viewport, bool showOverlays)
        {
            ...
            // Auto-track nodes
            foreach (SceneNode sn in this.autoTrackingSceneNodes.Values)
            {
                sn.AutoTrack();
            }
            ...
            if (IsShadowTechniqueInUse && this.illuminationStage != IlluminationRenderStage.RenderToTexture &&
                viewport.ShowShadows &&
        ...
        protected internal virtual void CreateShadowTextures(ushort size, ushort count, PixelFormat format)
        {
            ...
            if (!IsShadowTechniqueTextureBased ||
                this.shadowTextures.Count > 0 && count == this.shadowTextureCount && size == this.shadowTextureSize &&
                format == this.shadowTextureFormat)
            {
                // no change
                return;
            ...
            for (ushort t = 0; t < count; ++t)
            {
                ...
                Texture shadowTex = TextureManager.Instance[targName];
                if (shadowTex == null)
                {
        ...
        protected internal virtual void PrepareShadowTextures(Camera camera, Viewport viewPort)
        {
            ...
            if (!IsShadowTechniqueAdditive)
            {
                ...
                if (this.shadowTextureCustomReceiverPass != null)
                {
                    this.shadowTextureCustomReceiverPass.SetFog(true, FogMode.Linear, ColorEx.White, 0, fadeStart,
                                                                fadeEnd);
                }
            }
            else
        ...
        protected virtual void UpdateGpuProgramParameters(Pass pass)
        {
            if (pass.IsProgrammable)
            {
                ...
                if (pass.HasVertexProgram)
                {
                    this.targetRenderSystem.BindGpuProgramParameters(GpuProgramType.Vertex, pass.VertexProgramParameters,
                                                                     this._gpuParamsDirty);
                }

                if (pass.HasGeometryProgram)
                {
                    this.targetRenderSystem.BindGpuProgramParameters(GpuProgramType.Geometry,
                                                                     pass.GeometryProgramParameters,
                                                                     this._gpuParamsDirty);
                }
                                                  ...
                                                  LightList manualLightList)
        {
            ...
            if (!this.suppressRenderStateChanges)
            {
                ...
                for (int i = 0; i < pass.TextureUnitStatesCount; i++)
                {
                    TextureUnitState texUnit = pass.GetTextureUnitState(i);
                ...
                if (renderable.PolygonModeOverrideable)
                {
                                                  ...
                                                  LightList manualLightList)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    IRenderable renderable = renderables[r];
        ...
        protected virtual bool ValidateRenderableForRendering(Pass pass, IRenderable renderable)
        {
            ...
            {
                if (this.illuminationStage == IlluminationRenderStage.RenderReceiverPass && renderable.CastsShadows &&
        ...
        protected virtual void RenderQueueGroupObjects(RenderQueueGroup group)
        {
            ...
            bool doShadows = group.ShadowsEnabled && this.currentViewport.ShowShadows && !this.suppressShadows &&
                             !this.suppressRenderStateChanges;
        ...
        ///</summary>
        ///<remarks>
        ///  <p>Note that since directional lights have no position, they are always considered
        ///    closer than any point lights and as such will always take precedence.</p>
        ///  <p>Subclasses of the default SceneManager may wish to take into account other issues
        ...
        ///    those lights which are out of range.</p>
        ///  <p>The number of items in the list max exceed the maximum number of lights supported
        ///    by the renderer, but the extraneous ones will never be used. In fact the limit will
        ///    be imposed by Pass::getMaxSimultaneousLights.</p>
        ///</remarks>
        ...
        ///<param name="bow"> If above zero, the plane will be curved, allowing the sky to appear below camera level. Curved sky planes are simular to skydomes, but are more compatable with fog. </param>
        ///<param name="groupName"> </param>
                                        ...
                                        bool drawFirst, float bow, string groupName)
        {
            ...
            if (enable)
            {
                ...
                // set up the place
                Mesh planeMesh = MeshManager.Instance[meshName];
                ...
                if (bow > 0)
                {
                    planeMesh = MeshManager.Instance.CreateCurvedIllusionPlane(meshName, groupName, plane, scale*100,
                                                                               scale*100,
                                                                               scale*bow*100, 6, 6, false, 1, tiling,
                                                                               tiling, up);
                }
                else
                {
                    planeMesh = MeshManager.Instance.CreatePlane(meshName, groupName, plane, scale*100, scale*100, 1, 1,
                                                                 false, 1,
                                                                 tiling, tiling, up);
                }
        ...
        /// </summary>
        /// <note>Throws an exception if the named instance does not exist</note>
            ...
            public bool OnQueryResult(MovableObject sceneObject)
            {
                    ...
                    this.sceneManager.SpecialCaseRenderQueueList.IsRenderQueueToBeProcessed(sceneObject.RenderQueueGroup))
                {
                    ...
                    if (!this.isLightInFrustum || this.light.Type == LightType.Directional)
                    {
                        ...
                        {
                            PlaneBoundedVolume pbv = this.lightClipVolumeList[i];
        ...
        /// <returns> The default is to return 0, ie to not report progress. </returns>
        ...
        /// </remarks>
        /// <param name="stream"> Data stream containing data to load </param>
        /// <param name="typeName"> Identifies the type of world geometry contained in the stream - not required if this manager only supports one type of world geometry. </param>
        /// <returns> </returns>
    ...
    public class DefaultAxisAlignedBoxRegionSceneQuery : AxisAlignedBoxRegionSceneQuery
    {
        ...
        public override void Execute(ISceneQueryListener listener)
        {
            ...
            {
                MovableObjectCollection movableObjects = creator.GetMovableObjectCollection(map.Value.Type);
        ...
        public override void Execute(IRaySceneQueryListener listener)
        {
            ...
            foreach (KeyValuePair<string, MovableObjectFactory> map in factories)
            {
                ...
                foreach (MovableObject movableObject in movableObjects.Values)
                {
                    ...
                    IntersectResult results = Utility.Intersects(ray, movableObject.GetWorldBoundingBox());
        ...
        public override void Execute(ISceneQueryListener listener)
        {
            ...
            foreach (KeyValuePair<string, MovableObjectFactory> map in factories)
            {
                ...
                foreach (MovableObject movableObject in movableObjects.Values)
                {
                    ...
                    {
                        PlaneBoundedVolume volume = volumes[v];
                        // Do AABB / plane volume test
                        if ((movableObject.QueryFlags & queryMask) != 0 &&
    ...
    public class DefaultIntersectionSceneQuery : IntersectionSceneQuery
    {
        ...
        public override void Execute(IIntersectionSceneQueryListener listener)
        {
            ...
            {
                KeyValuePair<string, MovableObjectFactory> map =
                    (KeyValuePair<string, MovableObjectFactory>) enumFactories.Current;
                MovableObjectCollection movableObjects = creator.GetMovableObjectCollection(map.Value.Type);
                IEnumerator<MovableObject> enumA = movableObjects.GetEnumerator();
                while (enumA.Current != null)
                {
                    MovableObject objectA = enumA.Current;
                    // skip group if query type doesn't match
                    ...
                    IEnumerator<MovableObject> enumB = enumA;
                    while (enumB.Current != null)
                    {
                        MovableObject objectB = enumB.Current;
                    ...
                    {
                        KeyValuePair<string, MovableObjectFactory> mapLater =
                            (KeyValuePair<string, MovableObjectFactory>) enumFactoriesLater.Current;

                        MovableObjectCollection movableObjectsLater =
                            creator.GetMovableObjectCollection(mapLater.Value.Type);
                        IEnumerator<MovableObject> enumC = movableObjectsLater.GetEnumerator();
                        while (enumC.Current != null)
                        {
                            MovableObject objectC = enumC.Current;
                            // skip group if query type doesn't match
