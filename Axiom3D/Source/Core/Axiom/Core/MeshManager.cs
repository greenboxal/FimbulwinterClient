...
//     <id value="$Id: MeshManager.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Core
{
    ...
    public class MeshManager : ResourceManager, IManualResourceLoader
    {
        ...
        {
            get { return Singleton<MeshManager>.Instance; }
        }
        ...
        private readonly ChainedEvent<MeshSerializerArgs> _processMaterialNameEvent =
            new ChainedEvent<MeshSerializerArgs>();
        ...
        {
            add { this._processMaterialNameEvent.EventSinks += value; }
            remove { this._processMaterialNameEvent.EventSinks -= value; }
        }

        private readonly ChainedEvent<MeshSerializerArgs> _processSkeletonNameEvent =
            new ChainedEvent<MeshSerializerArgs>();
        ...
        {
            add { this._processSkeletonNameEvent.EventSinks += value; }
            remove { this._processSkeletonNameEvent.EventSinks -= value; }
        }
        ...
        {
            Mesh mesh = null;
            ulong handle = (ResourceHandle) name.ToLower().GetHashCode();
        ...
        public Mesh CreatePlane(string name, string group, Plane plane, float width, float height, int xSegments,
                                int ySegments, bool normals, int texCoordSetCount, float uTile, float vTile,
                                Vector3 upVec)
        {
            return CreatePlane(name, group, plane, width, height, xSegments, ySegments, normals, texCoordSetCount, uTile,
                               vTile,
        ...
        {
            return CreateCurvedPlane(name, group, plane, width, height, bow, xSegments, ySegments, normals,
                                     texCoordSetCount,
                                     xTile, yTile, upVec, BufferUsage.StaticWriteOnly, BufferUsage.StaticWriteOnly, true,
                                     true);
        }
        ...
        public Mesh CreateCurvedIllusionPlane(string name, string group, Plane plane, float width, float height,
                                              ...
                                              BufferUsage indexBufferUsage, bool vertexShadowBuffer,
                                              bool indexShadowBuffer)
        {
            return CreateCurvedIllusionPlane(name, group, plane, width, height, curvature, xSegments, ySegments, normals,
                                             texCoordSetCount, xTiles, yTiles, upVector, Quaternion.Identity,
                                             BufferUsage.StaticWriteOnly, BufferUsage.StaticWriteOnly,
                                             vertexShadowBuffer,
                                             indexShadowBuffer, -1);
        }
        ...
        /// </summary>
        public PatchMesh CreateBezierPatch(string name, string group, Array controlPointBuffer,
                                           ...
                                           bool vbUseShadow, bool ibUseShadow)
        {
            ...
            mesh.Define(controlPointBuffer, declaration, width, height, uMaxSubdivisionLevel, vMaxSubdivisionLevel,
                        visibleSide,
        ...
        ///</summary>
        private void _tesselate2DMesh(SubMesh subMesh, int width, int height, bool doubleSided,
                                      ...
                                      bool indexShadowBuffer)
        {
            ...
            BufferBase data = idxBuffer.Lock(BufferLocking.Discard);

#if !AXIOM_SAFE_ONLY
            unsafe
#endif
            {
                short* pIndex = data.ToShortPointer();
                int idx = 0;
                                              ...
                                              ref Vector3 max, ref float maxSquaredLength)
        {
...
#endif
            {
                ...
                float* pData = data.ToFloatPointer();
                int idx = 0;
        ...
        private void _generateCurvedPlaneVertexData(HardwareVertexBuffer vbuf, int ySegments, int xSegments,
                                                    float xSpace,
                                                    float halfWidth, float ySpace, float halfHeight, Matrix4 transform,
                                                    bool firstTime, bool normals, Matrix4 rotation, float curvature,
                                                    int numTexCoordSets, float xTexCoord, float yTexCoord,
                                                    SubMesh subMesh,
...
#endif
            {
                ...
                for (int y = 0; y <= ySegments; y++)
                {
                    for (int x = 0; x <= xSegments; x++)
                    {
                        ...
                        Real diff_x = (x - ((Real) xSegments/2))/xSegments;
                        Real diff_y = (y - ((Real) ySegments/2))/ySegments;
        ...
        private void _generateCurvedIllusionPlaneVertexData(HardwareVertexBuffer vertexBuffer, int ySegments,
                                                            int xSegments,
                                                            float xSpace, float halfWidth, float ySpace,
                                                            float halfHeight,
                                                            ...
                                                            ref Vector3 max, ref float maxSquaredLength)
        {
            ...
            {
                // lock the vertex buffer
                BufferBase data = vertexBuffer.Lock(BufferLocking.Discard);
        ...
        private void _loadManual(Mesh mesh, MeshBuildParams mbp)
        {
            ...
            HardwareVertexBuffer vbuf = HardwareBufferManager.Instance.CreateVertexBuffer(decl.Clone(0),
                                                                                          ...
                                                                                          mbp.VertexBufferUsage,
                                                                                          mbp.VertexShadowBuffer);
            ...
            switch (mbp.Type)
            {
                case MeshBuildType.Plane:
                    _generatePlaneVertexData(vbuf, mbp.YSegments, mbp.XSegments, xSpace, halfWidth, ySpace, halfHeight,
                                             transform,
                                             firstTime, mbp.Normals, rotation, mbp.TexCoordSetCount, xTexCoord,
                                             yTexCoord, subMesh,
                                             ref min, ref max, ref maxSquaredLength);
                    break;
                case MeshBuildType.CurvedPlane:
                    _generateCurvedPlaneVertexData(vbuf, mbp.YSegments, mbp.XSegments, xSpace, halfWidth, ySpace,
                                                   halfHeight,
                                                   transform, firstTime, mbp.Normals, rotation, mbp.Curvature,
                                                   mbp.TexCoordSetCount,
                                                   xTexCoord, yTexCoord, subMesh, ref min, ref max, ref maxSquaredLength);
                    break;
                case MeshBuildType.CurvedIllusionPlane:
                    _generateCurvedIllusionPlaneVertexData(vbuf, mbp.YSegments, mbp.XSegments, xSpace, halfWidth, ySpace,
                                                           halfHeight,
                                                           transform, firstTime, mbp.Normals, mbp.Orientation,
                                                           mbp.Curvature,
            ...
            // generate face list
            _tesselate2DMesh(subMesh, mbp.XSegments + 1, mbp.YSegments + 1, false, mbp.IndexBufferUsage,
                             mbp.IndexShadowBuffer);
        ...
        protected override Resource _create(string name, ulong handle, string group, bool isManual,
                                            ...
                                            Axiom.Collections.NameValuePairList createParams)
        {
		...
		public Mesh CreateBoneMesh( string name )
		{
			...
			{
				// lock the vertex buffer
				var data = vertexBuffer.Lock( BufferLocking.Discard );
        ...
        {
            get { return (Mesh) base[name]; }
        }

        public new Mesh this[ResourceHandle handle]
        {
            get { return (Mesh) base[handle]; }
        }
        ...
        public void LoadResource(Resource resource)
        {
            ...
            if (!prefab)
            {
                ...
                MeshBuildParams parameters = this._meshBuildParams[mesh];
