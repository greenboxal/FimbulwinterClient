...
//     <id value="$Id: Entity.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Core
{
    ...
    ///  <para> The <see cref="Mesh" /> and <see cref="SubMesh" /> classes deal with the definition of the geometry used by discrete movable objects. Entities are actual instances of objects based on this geometry in the world. Therefore there is usually a single set <see
    ///   cref="Mesh" /> for a car, but there may be multiple entities based on it in the world. Entities are able to override aspects of the Mesh it is defined by, such as changing material properties per instance (so you can have many cars using the same geometry but different textures for example). Because a <see
    ///   cref="Mesh" /> is split into a list of <see cref="SubMesh" /> objects for this purpose, the Entity class is a grouping class (much like the <see
    ///   cref="Mesh" /> class) and much of the detail regarding individual changes is kept in the <see cref="SubEntity" /> class. There is a 1:1 relationship between <see
    ///   cref="SubEntity" /> instances and the <see cref="SubMesh" /> instances associated with the <see cref="Mesh" /> the Entity is based on. </para>
    ///  <para> Entity and <see cref="SubEntity" /> classes are never created directly. Use <see
    ///   cref="SceneManager.CreateEntity(string, string)" /> (passing a model name) to create one. </para>
    ///  <para> Entities are included in the scene by using <see cref="SceneNode.AttachObject" /> to associate them with a scene node. </para>
    ///</remarks>
    public class Entity : MovableObject
    {
        ...
        protected void SetMesh(Mesh mesh)
        {
            ...
            if (mesh.IsLodManual)
            {
                for (int i = 1; i < mesh.LodLevelCount; i++)
                {
                    ...
                    Entity lodEnt = new Entity(string.Format("{0}Lod{1}", name, i), usage.ManualMesh);
        ...
        {
            get { return this.numBoneMatrices; }
        }
        ...
        {
            get { return this.skelAnimVertexData; }
        }
        ...
        public bool HasEnabledAnimationState
        {
            get
            {
                foreach (KeyValuePair<string, AnimationState> item in this.animationState)
                {
        ...
        {
            get { return this.displaySkeleton; }
            set { this.displaySkeleton = value; }
        }
        ...
        {
            get { return this.skeletonInstance != null; }
        }
        ...
        {
            get { return this.subEntityList.Count; }
        }
        ...
        {
            get { return this.softwareVertexAnimVertexData; }
        }
        ...
        {
            get { return this.hardwareVertexAnimVertexData; }
        }
        ...
        {
            get { return this.vertexAnimationAppliedThisFrame; }
        }
        ...
        {
            get { return this.hardwareAnimation; }
        }
                                           ...
                                           Vector3 offsetPosition)
        {
            ...
            if (bone == null)
            {
                throw new AxiomException("Entity '{0}' does not have a skeleton with a bone named '{1}'.", name,
                                         boneName);
            }
        ...
        {
            if (this.softwareAnimationRequests == 0 || (normalsAlso && this.softwareAnimationNormalsRequests == 0))
            {
                ...
                if (HasSkeleton)
                {
                    ...
                    if (swAnimation)
                    {
                        ...
                        if (this.skelAnimVertexData != null)
                        {
                            ...
                            Mesh.SoftwareVertexBlend(
                                (this.mesh.SharedVertexDataAnimationType != VertexAnimationType.None
                                     ...
                                     : this.mesh.SharedVertexData), this.skelAnimVertexData, this.boneMatrices,
                                blendNormals, blendTangents,
                                blendBinormals);
                        }
                        ...
                        foreach (SubEntity subEntity in this.subEntityList)
                        {
                            ...
                            {
                                subEntity.TempSkelAnimInfo.CheckoutTempCopies(true, blendNormals, blendTangents,
                                                                              blendBinormals);
                                ...
                                Mesh.SoftwareVertexBlend(
                                    (subEntity.SubMesh.VertexAnimationType != VertexAnimationType.None
                                         ...
                                         : subEntity.SubMesh.vertexData), subEntity.SkelAnimVertexData,
                                    this.boneMatrices, blendNormals,
                                    blendTangents,
                                    blendBinormals);
                            }
                        }
        ...
        private void ApplyVertexAnimation(bool hardwareAnimation, bool stencilShadows)
        {
            ...
            if (hardwareAnimation)
            {
                if (this.hardwareVertexAnimVertexData != null &&
                    this.mesh.SharedVertexDataAnimationType != VertexAnimationType.None)
                {
                    InitHardwareAnimationElements(this.hardwareVertexAnimVertexData,
        ...
        protected void RestoreBuffersForUnusedAnimation(bool hardwareAnimation)
        {
            ...
            if (this.mesh.SharedVertexData != null && !this.vertexAnimationAppliedThisFrame &&
                (!hardwareAnimation || this.mesh.SharedVertexDataAnimationType == VertexAnimationType.Morph))
            {
                VertexElement srcPosElem =
                    this.mesh.SharedVertexData.vertexDeclaration.FindElementBySemantic(VertexElementSemantic.Position);
        ...
        public VertexData GetVertexDataForBinding()
        {
            VertexDataBindChoice c =
                ChooseVertexDataForBinding(this.mesh.SharedVertexDataAnimationType != VertexAnimationType.None);
            switch (c)
            {
        ...
        {
            get { return this.meshLodIndex; }
            set { this.meshLodIndex = value; }
        }
        ...
        {
            get { return (uint) SceneQueryTypeMask.Entity; }
        }
                                                                        ...
                                                                        float extrusionDistance, int flags)
        {
            ...
            if (this.meshLodIndex > 0 && this.mesh.IsLodManual)
            {
                ...
                return lodEnt.GetShadowVolumeRenderableEnumerator(technique, light, indexBuffer, extrudeVertices,
                                                                  extrusionDistance,
                                                                  flags);
            }
            ...
            for (int i = 0; i < this.shadowRenderables.Capacity; i++)
            {
                ...
                if (init)
                {
                    ...
                    esr = new EntityShadowRenderable(this, indexBuffer, data,
                ...
                if (p.HasVertexProgram)
                {
                    ...
                    else if (animType == VertexAnimationType.Pose)
                    {
                        ...
                        else
                        {
                            ...
                            if (subEntity.SubMesh.useSharedVertices)
                            {
                                this.hardwarePoseCount = Utility.Max(this.hardwarePoseCount,
                                                                     p.VertexProgram.PoseAnimationCount);
                            }
                            else
                            {
                                subEntity.HardwarePoseCount =
                                    Utility.Max(subEntity.HardwarePoseCount, p.VertexProgram.PoseAnimationCount);
                            }
                        }
                    }
                }
        ...
        protected class EntityShadowRenderable : ShadowRenderable
        {
            ...
            {
                get { return this.positionBuffer; }
            }
            ...
            {
                get { return this.parent.ParentNode.DerivedOrientation; }
            }

            public override Vector3 WorldPosition
            {
                get { return this.parent.ParentNode.DerivedPosition; }
            }
        ...
        protected override MovableObject _createInstance(string name, NamedParameterList param)
        {
            ...
            if (param != null)
            {
                if (param.ContainsKey("mesh"))
                {
                    ...
                    {
                        pMesh = MeshManager.Instance.Load(param["mesh"].ToString(),
