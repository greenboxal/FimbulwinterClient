...
//     <id value="$Id: BillboardChain.cs 3315 2012-05-31 20:37:22Z borrillis $"/>
// </file>
...
namespace Axiom.Core
{
    public class BillboardChain : MovableObject, IRenderable
    {
        ...
        {
            get { return this.texCoordDirection; }
            set { this.texCoordDirection = value; }
        }

        public virtual float[] OtherTexCoordRange
        {
            get { return this.otherTexCoordRange; }
            set { this.otherTexCoordRange = value; }
        }
        ...
        protected virtual void SetupBuffers()
        {
            ...
            if (this.buffersNeedRecreating)
            {
                ...
                HardwareVertexBuffer buffer =
                    HardwareBufferManager.Instance.CreateVertexBuffer(this.vertexData.vertexDeclaration.Clone(0),
        ...
        protected virtual void UpdateVertexBuffer(Camera camera)
        {
            ...
            Vector3 eyePosition = ParentNode.DerivedOrientation.Inverse()*(camPosition - ParentNode.DerivedPosition)/
...
#endif
            {
                BufferBase bufferStart = bufferPtr;
                ...
                foreach (ChainSegment segment in this.chainSegmentList)
                {
                    ...
                    if (segment.head != SEGMENT_EMPTY && segment.head != segment.tail)
                    {
                        ...
                        for (int e = segment.head;; ++e)
                        {
                            ...
                            BufferBase pBase = bufferStart + buffer.VertexSize*baseIndex;
                            ...
                            float* pFloat = pBase.ToFloatPointer();
                            // pos1
                            pFloat[0] = pos0.x;
                            pFloat[1] = pos0.y;
                            pFloat[2] = pos0.z;
        ...
        protected virtual void UpdateIndexBuffer()
        {
            ...
            {
                BufferBase pBufferBase = this.indexData.indexBuffer.Lock(BufferLocking.Discard);
...
#endif
                {
                    ushort* pShort = pBufferBase.ToUShortPointer();
                    int idx = 0;
                    // indexes
                    foreach (ChainSegment segment in this.chainSegmentList)
                    {
        ...
        protected virtual void UpdateBoundingBox()
        {
            if (this.boundsDirty)
            {
                ...
                else
                {
                    this.radius =
                        (float)
                        Utility.Sqrt(Utility.Max(this.aabb.Minimum.LengthSquared, this.aabb.Maximum.LengthSquared));
                }
