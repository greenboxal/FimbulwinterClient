...
//     <id value="$Id: InputReader.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Input
{
    ...
    public abstract class InputReader : IDisposable
    {
        ...
        ///<param name="ownMouse"> If true, input will be taken over from the OS and exclusive to the window. If false, input will still be shared with other apps. </param>
        ///<param name="useKeyboard"> </param>
        ///<param name="useMouse"> </param>
        public abstract void Initialize(RenderWindow parent, bool useKeyboard, bool useMouse, bool useGamepad,
                                        bool ownMouse);
        ...
        ///</summary>
        ///<value> When true, events will be fired when keyboard input occurs on a call to <see cref="Capture" /> . When false, the current keyboard state will be available via <see
        ///   cref="IsKeyPressed" /> . </value>
        public abstract bool UseKeyboardEvents { get; set; }

        ///<summary>
        ///  Get/Set whether or not to use event based mouse input notification.
        ///</summary>
        ///<value> When true, events will be fired when mouse input occurs on a call to <see cref="Capture" /> . When false, the current mouse state will be available via <see
        ///   cref="IsMousePressed" /> . </value>
        ...
        protected void KeyChanged(KeyCodes key, bool down)
        {
            if (down)
            {
                ...
                KeyEventArgs e = new Axiom.Input.KeyEventArgs(key, this.modifiers);
                OnKeyDown(e);
            }
            else
            {
