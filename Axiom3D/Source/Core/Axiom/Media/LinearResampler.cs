...
namespace Axiom.Media
{
    ...
    public partial class LinearResampler
    {
        ...
        public void Scale(PixelBox src, PixelBox dst)
        {
            ...
            ulong stepx = ((UInt64) src.Width << 48)/(UInt64) dst.Width;
            ulong stepy = ((UInt64) src.Height << 48)/(UInt64) dst.Height;
            ulong stepz = ((UInt64) src.Depth << 48)/(UInt64) dst.Depth;
            ...
            for (int z = dst.Front; z < dst.Back; z++, sz_48 += stepz)
            {
                ...
                for (int y = dst.Top; y < dst.Bottom; y++, sy_48 += stepy)
                {
                    ...
                    for (int x = dst.Left; x < dst.Right; x++, sx_48 += stepx)
                    {
                        ...
                        ColorEx accum = x1y1z1*((1.0f - sxf)*(1.0f - syf)*(1.0f - szf)) +
        ...
        private void Unpack(ref ColorEx dst, int x, int y, int z, PixelFormat format, BufferBase src, PixelBox srcbox,
                            int elemsize)
        {
            BufferBase data = src + (elemsize*((x) + (y)*srcbox.RowPitch + (z)*srcbox.SlicePitch));
