...
//     <id value="$Id: DDSCodec.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Media
{
    ...
    public class DDSCodec : ImageCodec
    {
        ...
        private struct DXTColorBlock
        {
            ...
            {
                DXTColorBlock cBlock = new DXTColorBlock();
        ...
        private struct DXTExplicitAlphaBlock
        {
            ...
            {
                DXTExplicitAlphaBlock block = new DXTExplicitAlphaBlock();
        ...
        private struct DXTInterpolatedAlphaBlock
        {
            ...
            {
                DXTInterpolatedAlphaBlock block = new DXTInterpolatedAlphaBlock();
        ...
        public override void EncodeToFile(Stream input, string outFileName, Codec.CodecData data)
        {
            ...
            if (notImplemented)
            {
                throw new NotImplementedException(string.Format("DDS encoding for{0} not supported",
            ...
            else
            {
                ...
                // Write the file
                using (
                    BinaryWriter br = new BinaryWriter(File.Open(outFileName, FileMode.OpenOrCreate, FileAccess.Write)))
                {
                    br.Write(ddsMagic);
                    ddsHeader.Write(br);
        ...
        private void _unpackDXTAlpha(DXTInterpolatedAlphaBlock block, ColorEx[] pCol)
        {
            ...
            for (int i = 0; i < 16; ++i)
            {
                ...
                if (baseBit > 5)
                {
                    byte extraBits = (byte) ((block.indexes[baseByte + 1] << (8 - baseBit)) & 0xFF);
                    bits |= extraBits & 0x7;
                }
        ...
        public override Codec.DecodeResult Decode(Stream input)
        {
            using (BinaryReader br = new BinaryReader(input))
            {
                ...
                byte[] dest = new byte[imgData.size];
                    ...
                    for (int mip = 0; mip <= imgData.numMipMaps; ++mip)
                    {
                        ...
                        if (PixelUtil.IsCompressed(sourceFormat))
                        {
                            ...
                            if (decompressDXT)
                            {
                                ...
                                ColorEx[] tempColours = new ColorEx[16];
                                    ...
                                    for (int y = 0; y < height; y += 4)
                                    {
                                        for (int x = 0; x < width; x += 4)
                                        {
                                            ...
                                            for (int by = 0; by < sy; ++by)
                                            {
                                                ...
                                                {
                                                    PixelConverter.PackColor(tempColours[by*4 + bx], imgData.format,
                                                                             destBuffer);
                                                    destBuffer += destBpp;
                                                }
                                                // advance to next row
                        ...
                        else
                        {
                            ...
                            long srcAdvance = (long) (srcPitch - dstPitch);
