...
//     <id value="$Id: Image.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Media
{
    ...
    public class Image : DisposableObject
    {
        ...
        {
            get { return PixelUtil.GetNumElemBits(this.format); }
        }
        ...
        public int RowSpan
        {
            get { return this.width*PixelUtil.GetNumElemBytes(this.format); }
        }
...
#endif
            {
                byte* srcBytes = bufPtr.ToBytePointer();
        ...
        public void FlipAroundY()
        {
            ...
            int bytes = PixelUtil.GetNumElemBytes(this.format);
            byte[] tempBuffer = new byte[this.width*this.height*bytes];
        ...
        public void FlipAroundX()
        {
            ...
            int bytes = PixelUtil.GetNumElemBytes(this.format);
            this.numMipMaps = 0; // Image operations lose precomputed mipmaps
            int rowSpan = this.width*bytes;

            byte[] tempBuffer = new byte[rowSpan*this.height];
        ...
        public static Image FromFile(string fileName, string groupName)
        {
            ...
            Stream encoded = ResourceGroupManager.Instance.OpenResource(fileName, groupName);
    ...
    /// <see cref="Image.FromDynamicImage(byte[], int, int, int, PixelFormat, bool, int, int)" />
...
#endif
        {
            ...
            else
            {
                ...
                int magicLen = Axiom.Math.Utility.Min((int) stream.Length, 32);
            ...
            Codec.DecodeResult res = codec.Decode(stream);
            MemoryStream decoded = (MemoryStream) res.First;
            ImageCodec.ImageData data = (ImageCodec.ImageData) res.Second;
    ...
    /// <see cref="Image.FromStream(Stream, string)" />
        ...
        public static string GetFileExtFromMagic(Stream stream)
        {
            ...
            // return to start
            stream.Position = 0;
            Codec codec = CodecManager.Instance.GetCodec(magicBuf, magicLen);
        ...
        public void Save(String filename)
        {
            ...
            Codec pCodec = CodecManager.Instance.GetCodec(strExt);
            if (pCodec == null)
            {
                throw new AxiomException("Unable to save image file '{0}' - invalid extension.", filename);
            }
            ...
            MemoryStream wrapper = new MemoryStream(this.buffer);
                            ...
                            switch (PixelUtil.GetNumElemBytes(src.Format))
                            {
                                ...
                                default:
                                    throw new NotSupportedException(
                                        String.Format("Scaling of images using {0} byte format is not supported.",
