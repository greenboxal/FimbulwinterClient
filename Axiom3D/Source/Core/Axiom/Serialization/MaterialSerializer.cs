...
//     <id value="$Id: MaterialSerializer.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.Serialization
{
    ...
    public class MaterialSerializer
    {
        ...
        {
            string[] splitCmd = StringConverter.Split(line, new[]
                                                                {
                                                                    ' ', '\t'
                                                                }, 2);
            ...
            if (parsers.ContainsKey(splitCmd[0]))
            {
                ...
                MethodInfo handler = parsers[splitCmd[0]];
        ...
        {
            MaterialScriptProgramDefinition def = this.scriptContext.programDef;
            GpuProgram gp = null;
            ...
            if (def.language == "asm")
            {
                ...
                {
                    LogParseError(this.scriptContext,
                                  "Invalid program definition for {0}, you must specify a source file.", def.name);
                }
                if (def.syntax == string.Empty)
                {
                    LogParseError(this.scriptContext,
                                  "Invalid program definition for {0}, you must specify a syntax code.", def.name);
                }

                // create
                gp = GpuProgramManager.Instance.CreateProgram(def.name, this.scriptContext.groupName, def.source,
                                                              def.progType,
                                                              def.syntax);
            }
            else
            {
                ...
                try
                {
                    HighLevelGpuProgram hgp = HighLevelGpuProgramManager.Instance.CreateProgram(def.name,
                                                                                                this.scriptContext.
                                                                                                    groupName,
                                                                                                def.language,
                                                                                                def.progType);
                ...
                {
                    LogManager.Instance.Write("Could not create GPU program '{0}'. error reported was: {1}.", def.name,
                                              ex.Message);
                    return;
                }
            }
            ...
            if (gp.IsSupported && this.scriptContext.defaultParamLines.Count > 0)
            {
                ...
                for (int i = 0; i < this.scriptContext.defaultParamLines.Count; i++)
                {
                    ...
                    string[] splitCmd = StringConverter.Split(this.scriptContext.defaultParamLines[i], new[]
                                                                                                           {
                                                                                                               ' ', '\t'
                                                                                                           }, 2);
                    ...
                    if (this.programDefaultParamAttribParsers.ContainsKey(splitCmd[0]))
                    {
                        ...
                        MethodInfo handler = this.programDefaultParamAttribParsers[splitCmd[0]];
                        // Use parser, make sure we have 2 params before using splitCmd[1]
                ...
                case MaterialScriptSection.Program:
                    ...
                    else
                    {
                        ...
                        if (this.programAttribParsers.ContainsKey(splitCmd[0]))
                        {
                            ...
                            MethodInfo handler = this.programAttribParsers[splitCmd[0]];
                            return (bool) handler.Invoke(null, new object[]
                                                                   {
                                                                       cmd, this.scriptContext
                                                                   });
        ...
        {
            MethodInfo[] methods = GetType().GetMethods(BindingFlags.NonPublic | BindingFlags.Static);
            ...
            for (int i = 0; i < methods.Length; i++)
            {
                ...
                MaterialAttributeParserAttribute[] parserAtts =
                    (MaterialAttributeParserAttribute[])
                    method.GetCustomAttributes(typeof (MaterialAttributeParserAttribute), true);
        ...
        protected static bool ParseProgramCustomParameter(string parameters, MaterialScriptContext context)
        {
            ...
            string[] values = StringConverter.Split(parameters, new[]
                                                                    {
                                                                        ' ', '\t'
                                                                    }, 2);
        ...
        protected static bool ParseMaterial(string parameters, MaterialScriptContext context)
        {
            ...
            string[] values = parameters.Split(new[]
                                                   {
                                                       ':'
                                                   });
        ...
        protected static bool ParseAmbient(string parameters, MaterialScriptContext context)
        {
            ...
            if (values.Length == 1)
            {
                if (values[0].ToLower() == "vertexcolour" || values[0].ToLower() == "vertexcolor")
                {
                ...
                {
                    LogParseError(context,
                                  "Bad ambient attribute, single parameter flag must be 'vertexcolour' or 'vertexcolor'.");
                }
            }
        ...
        protected static bool ParseDiffuse(string parameters, MaterialScriptContext context)
        {
            ...
            if (values.Length == 1)
            {
                ...
                {
                    LogParseError(context,
                                  "Bad diffuse attribute, single parameter flag must be 'vertexcolour' or 'vertexcolor'.");
                }
            }
            else if (values.Length == 3 || values.Length == 4)
            {
        ...
        protected static bool ParseEmissive(string parameters, MaterialScriptContext context)
        {
            ...
            if (values.Length == 1)
            {
                ...
                {
                    LogParseError(context,
                                  "Bad emissive attribute, single parameter flag must be 'vertexcolour' or 'vertexcolor'.");
                }
            }
            else if (values.Length == 3 || values.Length == 4)
            {
        ...
        {
            string[] values = parameters.ToLower().Split(new[]
                                                             {
                                                                 ' ', '\t'
                                                             });
            ...
            switch (values.Length)
            {
                case 1:
                    ...
                    else
                    {
                        ...
                        LogParseError(context, "Bad scene_blend attribute, valid values for the 2nd parameter are {0}.",
                                      legalValues);
                        return false;
                    }
                    break;
                case 2:
                    ...
                    else
                    {
                        ...
                        if (srcVal == null)
                        {
                            LogParseError(context, "Bad scene_blend attribute, valid src blend factor values are {0}.",
                                          legalValues);
                        }
                        if (destVal == null)
                        {
                            LogParseError(context, "Bad scene_blend attribute, valid dest blend factor values are {0}.",
                                          legalValues);
                        }
                    }
                    break;
        ...
        {
            string[] vecparams = parameters.ToLower().Split(new[]
                                                                {
                                                                    ' ', '\t'
                                                                });
            ...
            if (vecparams.Length == 2)
            {
                ...
                {
                    LogParseError(context, "Bad separate_scene_blend attribute, unrecognised parameter '{0}'",
                                  vecparams[0]);
                    return false;
                }
        ...
        protected static bool ParseVertexProgramRef(string parameters, MaterialScriptContext context)
        {
            ...
            {
                // unknown program
                LogParseError(context, "Invalid vertex_program_ref entry - vertex program {0} has not been defined.",
                              parameters);
                return true;
            }
        ...
        protected static bool ParseShadowCasterVertexProgramRef(string parameters, MaterialScriptContext context)
        {
            ...
            if (context.program == null)
            {
                // unknown program
                LogParseError(context,
                              "Invalid shadow_caster_vertex_program_ref entry - vertex program {0} has not been defined.",
                              parameters);
                return true;
            }
        ...
        protected static bool ParseAlphaOpEx(string parameters, MaterialScriptContext context)
        {
            ...
            try
            {
                ...
                if (op == LayerBlendOperationEx.BlendManual)
                {
                    ...
                    {
                        LogParseError(context,
                                      "Bad alpha_op_ex attribute, wrong number of parameters (expected 4 for manual blend).");
                        return false;
                    }
                ...
                if (src1 == LayerBlendSource.Manual)
                {
                    ...
                    {
                        LogParseError(context, "Bad alpha_op_ex attribute, wrong number of parameters (expected {0}).",
                                      paramIndex - 1);
                        return false;
                    }
        ...
        protected static bool ParseColorOpFallback(string parameters, MaterialScriptContext context)
        {
            ...
            else
            {
                ...
                {
                    LogParseError(context,
                                  "Bad color_op_multipass_fallback attribute, valid values for src value are {0}.",
                                  legalValues);
                }
                if (destVal == null)
                {
                    LogParseError(context,
                                  "Bad color_op_multipass_fallback attribute, valid values for dest value are {0}.",
                                  legalValues);
                }
            }
        ...
        protected static bool ParseColorOpEx(string parameters, MaterialScriptContext context)
        {
            ...
            try
            {
                ...
                if (src1 == LayerBlendSource.Manual)
                {
                    ...
                    {
                        LogParseError(context, "Bad color_op_ex attribute, wrong number of parameters (expected {0}).",
                                      paramIndex + 3);
                        return false;
                    }
        ...
        protected static bool ParseCubicTexture(string parameters, MaterialScriptContext context)
        {
            ...
            switch (uvw)
            {
                ...
                default:
                    LogParseError(context,
                                  "Bad cubic_texture attribute, last param must be 'combinedUVW' or 'separateUV'.");
                    return false;
            }
        ...
        protected static bool ParseFiltering(string parameters, MaterialScriptContext context)
        {
            ...
            else if (values.Length == 3)
            {
                ...
                if (val1 == null || val2 == null || val3 == null)
                {
                    ...
                    LogParseError(context, "Bad filtering attribute, valid values for filter options are {0}",
                                  legalValues);
                    return false;
                }
                else
                {
                    context.textureUnit.SetTextureFiltering((FilterOptions) val1, (FilterOptions) val2,
                                                            (FilterOptions) val3);
                }
            }
            else
        ...
        protected static bool ParseTexture(string parameters, MaterialScriptContext context)
        {
            ...
            int mipmaps = (int) TextureMipmap.Default;
            ...
            for (int p = 1; p < values.Length; p++)
            {
                switch (values[p].ToLower())
                {
                    ...
                    default:
                        ...
                        else if ((desiredFormat = PixelUtil.GetFormatFromName(values[p], true, false)) !=
                                 PixelFormat.Unknown)
                        {
                            // Nothing to do here.
                        }
        ...
        protected static bool ParseParamNamed(string parameters, MaterialScriptContext context)
        {
            ...
            try
            {
                GpuProgramParameters.GpuConstantDefinition def = context.programParams.GetConstantDefinition(values[0]);
            }
            catch (Exception ex)
            {
                                                        ...
                                                        MaterialScriptContext context, int index, string paramName)
        {
            ...
            {
                LogParseError(context, "Invalid {0} attribute - you need {1} parameters for a parameter of type {2}",
                              commandName,
                              2 + dims, type);
                return;
            }
            ...
            if (isReal)
            {
                float[] realBuffer = new float[roundedDims];
                ...
                if (isMatrix4x4)
                {
                    ...
                    Matrix4 m4X4 = new Matrix4(realBuffer[0], realBuffer[1], realBuffer[2], realBuffer[3], realBuffer[4],
                                               realBuffer[5], realBuffer[6], realBuffer[7], realBuffer[8], realBuffer[9],
                                               realBuffer[10], realBuffer[11], realBuffer[12], realBuffer[13],
                                               realBuffer[14],
                                               realBuffer[15]);
                    if (isNamed)
                    {
                                                      ...
                                                      MaterialScriptContext context, int index, string paramName)
        {
            ...
            {
                LogParseError(context, "Invalid " + commandName + " attribute - " + parameters[1]);
                return;
            }
                ...
                case GpuProgramParameters.AutoConstantDataType.Int:
                    {
                        ...
                        if (autoConstantDef.AutoConstantType ==
                            GpuProgramParameters.AutoConstantType.AnimationParametric)
                        {
                            if (isNamed)
                            {
                            ...
                            {
                                context.programParams.SetAutoConstant(index, autoConstantDef.AutoConstantType,
                                                                      context.numAnimationParametrics++);
                            }
                        }
                            // Special case texture projector - assume 0 if data not specified
                        else if ((autoConstantDef.AutoConstantType ==
                                  GpuProgramParameters.AutoConstantType.TextureViewProjMatrix ||
                                  autoConstantDef.AutoConstantType ==
                                  GpuProgramParameters.AutoConstantType.TextureWorldViewProjMatrix ||
                                  autoConstantDef.AutoConstantType ==
                                  GpuProgramParameters.AutoConstantType.SpotLightViewProjMatrix ||
                                  autoConstantDef.AutoConstantType ==
                                  GpuProgramParameters.AutoConstantType.SpotLightWorldViewProjMatrix) &&
                                 parameters.Length == 2)
                        {
                            if (isNamed)
                            {
                                context.programParams.SetNamedAutoConstant(paramName,
                                                                           autoConstantDef.AutoConstantType, 0);
                            }
                            else
                            {
                                context.programParams.SetAutoConstant(index, autoConstantDef.AutoConstantType, 0);
                        ...
                        else
                        {
                            ...
                            {
                                LogParseError(context,
                                              "Invalid " + commandName + " attribute - " + "expected 3 parameters.");
                                return;
                            }

                            int extraParam = int.Parse(parameters[2]);
                            if (isNamed)
                            {
                                context.programParams.SetNamedAutoConstant(paramName,
                                                                           autoConstantDef.AutoConstantType,
                ...
                case GpuProgramParameters.AutoConstantDataType.Real:
                    {
                            ...
                            autoConstantDef.AutoConstantType == GpuProgramParameters.AutoConstantType.FrameTime)
                        {
                            ...
                            {
                                context.programParams.SetAutoConstantReal(index, autoConstantDef.AutoConstantType,
                                                                          factor);
                            }
                        }
                        else // normal processing for auto constants that take an extra real value
                        {
                            ...
                            {
                                context.programParams.SetAutoConstantReal(index, autoConstantDef.AutoConstantType, rData);
                            }
                        }
                    }
                    break;
