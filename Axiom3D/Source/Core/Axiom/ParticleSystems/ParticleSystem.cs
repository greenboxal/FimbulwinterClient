...
//     <id value="$Id: ParticleSystem.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
...
namespace Axiom.ParticleSystems
{
    ...
    public class ParticleSystem : MovableObject
    {
		...
		*/
        ...
        protected Dictionary<int, List<ParticleEmitter>> emittedEmitterPool =
            new Dictionary<int, List<ParticleEmitter>>();
		...
		*/
        ...
        protected Dictionary<int, List<ParticleEmitter>> freeEmittedEmitters =
            new Dictionary<int, List<ParticleEmitter>>();
        ...
        protected void TriggerEmitters(float timeElapsed)
        {
            ...
            {
                ParticleEmitter activeEmitter = this.activeEmittedEmitters[index];
                executeTriggerEmitters(activeEmitter, (int) (activeEmitter.GetEmissionCount(timeElapsed)*ratio),
                                       timeElapsed);
            }
        }
        ...
        private void executeTriggerEmitters(ParticleEmitter emitter, int requested, float timeElapsed)
        {
            ...
            for (int j = 0; j < requested; ++j)
            {
                ...
                if (!this.localSpace)
                {
                    p.Position = (parentNode.DerivedOrientation*(parentNode.DerivedScale*p.Position)) +
        ...
        protected void ConfigureRenderer()
        {
            ...
            if (this.renderer != null && !this.isRendererConfigured)
            {
                ...
                Material mat = (Material) MaterialManager.Instance.Load(this.materialName, this.resourceGroupName);
        ...
        protected void SetBounds(AxisAlignedBox aabb)
        {
            ...
            float sqDist = (float) Utility.Max(this.aab.Minimum.LengthSquared, this.aab.Maximum.LengthSquared);
        ...
        {
            get { return this.localSpace; }
            set { this.localSpace = value; }
        }
        ...
        {
            get { return this.activeParticles.Count; }
        }
        ...
        {
            get { return this.activeParticles; }
        }
        ...
        {
            get { return this.cullIndividual; }
            set { this.cullIndividual = value; }
        }
        ...
        {
            get { return this.iterationInterval; }
            set
            {
        ...
        {
            get { return defaultIterationInterval; }
            set { defaultIterationInterval = value; }
        }

        public float NonVisibleUpdateTimeout
        {
            get { return this.nonvisibleTimeout; }
            set
            {
        ...
        {
            get { return defaultNonvisibleTimeout; }
            set { defaultNonvisibleTimeout = value; }
        }
        ...
        private void initializeEmittedEmitters()
        {
            ...
            {
                foreach (List<ParticleEmitter> peList in this.emittedEmitterPool.Values)
                {
        ...
        private void initializeEmittedEmitterPool()
        {
            ...
            foreach (ParticleEmitter emitter in this.emitterList)
            {
                ...
                if (emitter != null && emitter.EmittedEmitter != string.Empty)
                {
                    ...
                    this.emittedEmitterPool.Add(emitter.EmittedEmitter.ToLower().GetHashCode(),
        ...
        private void increaseEmittedEmitterPool(int size)
        {
            ...
            int maxNumberOfEmitters = size/this.emittedEmitterPool.Count;
            ...
            foreach (KeyValuePair<int, List<ParticleEmitter>> poolEntry in this.emittedEmitterPool)
            {
                e = poolEntry.Value;
                ...
                foreach (ParticleEmitter emitter in this.emitterList)
                {
                    if (emitter != null && poolEntry.Key == emitter.Name.ToLower().GetHashCode())
                    {
                        ...
                        for (int t = oldSize; t < maxNumberOfEmitters; ++t)
                        {
                            ...
                            clonedEmitter.IsEmitted = emitter.IsEmitted;
                                // is always 'true' by the way, but just in case
        ...
        {
            get { return (uint) SceneQueryTypeMask.Fx; }
        }
