...
namespace Axiom.RenderSystems.DirectX9
{
    ...
    public sealed class D3D9Texture : Texture, ID3D9Resource
    {
        ...
        public D3D9.BaseTexture DXTexture
        {
            ...
            get
            {
                D3D9.Device d3d9Device = D3D9RenderSystem.ActiveD3D9Device;
        ...
        public override void CopyTo(Texture target)
        {
            ...
            // get the target
            D3D9Texture other = (D3D9Texture) target;
            // target rectangle (whole surface)
            Rectangle dstRC = new System.Drawing.Rectangle(0, 0, other.Width, other.Height);

            foreach (KeyValuePair<D3D9.Device, TextureResources> it in this._mapDeviceToTextureResources)
            {
                    ...
                    dstTextureResources.NormalTexture != null)
                {
                    ...
                    D3D9.Surface srcSurface = srcTextureResources.NormalTexture.GetSurfaceLevel(0);

                    // get our target surface
                    D3D9.Surface dstSurface = dstTextureResources.NormalTexture.GetSurfaceLevel(0);

                    // do the blit, it's called StretchRect in D3D9 :)
                    Result res = it.Key.StretchRectangle(srcSurface, new System.Drawing.Rectangle(), dstSurface, dstRC,
                         ...
                         dstTextureResources.CubeTexture != null)
                {
                    ...
                    {
                        // get our source surface
                        D3D9.Surface srcSurface =
                            srcTextureResources.CubeTexture.GetCubeMapSurface((D3D9.CubeMapFace) face, 0);

                        // get our target surface
                        D3D9.Surface dstSurface =
                            dstTextureResources.CubeTexture.GetCubeMapSurface((D3D9.CubeMapFace) face, 0);

                        // do the blit, it's called StretchRect in D3D9 :)
                        Result res = it.Key.StretchRectangle(srcSurface, new System.Drawing.Rectangle(), dstSurface,
                                                             dstRC,
                                                             D3D9.TextureFilter.None);
                        if (res.Failure)
                        {
        ...
        protected override void load()
        {
            ...
            foreach (D3D9.Device d3d9Device in D3D9RenderSystem.ResourceCreationDevices)
            {
                _load(d3d9Device);
            }
        ...
        private void _load(D3D9.Device d3d9Device)
        {
            ...
            MemoryStream[] LoadedStreams = new MemoryStream[this._loadedStreams.Length];
        ...
        private MemoryStream[] _prepareVolumeTexture()
        {
            ...
            // find & load resource data
            Stream dstream = ResourceGroupManager.Instance.OpenResource(_name, _group, true, this);
            MemoryStream[] loadedStreams = new[]
                                               {
        ...
        private void _loadCubeTexture(D3D9.Device d3d9Device, MemoryStream[] loadedStreams)
        {
            ...
            if (GetSourceFileType() == "dds")
            {
                ...
                D3D9.Capabilities rkCurCaps = device.D3D9DeviceCaps;
                ...
                D3D9.Pool pool = UseDefaultPool() ? D3D9.Pool.Default : D3D9.Pool.Managed;
                ...
                try
                {
                    textureResources.CubeTexture = D3D9.CubeTexture.FromMemory(d3d9Device, loadedStreams[0].GetBuffer(),
                                                                               (int) loadedStreams[0].Length, numMips,
                                                                               d3dUsage,
                                                                               D3D9.Format.Unknown, pool,
                                                                               D3D9.Filter.Default,
                ...
                D3D9.SurfaceDescription texDesc = textureResources.CubeTexture.GetLevelDescription(0);
        ...
        private void _loadVolumeTexture(D3D9.Device d3d9Device, MemoryStream[] loadedStreams)
        {
            ...
            if (GetSourceFileType() == "dds")
            {
                ...
                try
                {
                    textureResources.VolumeTexture = D3D9.VolumeTexture.FromMemory(d3d9Device,
                                                                                   loadedStreams[0].GetBuffer(), -1, -1,
                                                                                   -1, // dims
                                                                                   numMips, d3dUsage,
                                                                                   D3D9.Format.Unknown, pool,
                                                                                   D3D9.Filter.Default,
                                                                                   D3D9.Filter.Default, 0
                        // colour key
                        );
                }
                catch (Exception ex)
                ...
                // set src and dest attributes to the same, we can't know
                D3D9.VolumeDescription texDesc = textureResources.VolumeTexture.GetLevelDescription(0);
                this._d3dPool = texDesc.Pool;
                // set src and dest attributes to the same, we can't know
        ...
        private void _loadNormalTexture(D3D9.Device d3d9Device, MemoryStream[] loadedStreams)
        {
            ...
            if (GetSourceFileType() == "dds")
            {
                ...
                try
                {
                    textureResources.NormalTexture = D3D9.Texture.FromMemory(d3d9Device, loadedStreams[0].GetBuffer(),
                                                                             -1, -1,
                                                                             // dims
                ...
                // set src and dest attributes to the same, we can't know
                D3D9.SurfaceDescription texDesc = textureResources.NormalTexture.GetLevelDescription(0);
                this._d3dPool = texDesc.Pool;
                // set src and dest attributes to the same, we can't know
                _setSrcAttributes(texDesc.Width, texDesc.Height, 1, D3D9Helper.ConvertEnum(texDesc.Format));
                _setFinalAttributes(d3d9Device, textureResources, texDesc.Width, texDesc.Height, 1,
        ...
        protected override void createInternalResources()
        {
            ...
            foreach (D3D9.Device i in D3D9RenderSystem.ResourceCreationDevices)
            {
        ...
        private void _createNormalTexture(D3D9.Device d3d9Device)
        {
            ...
            D3D9.Format d3dPF = _chooseD3DFormat(d3d9Device);

            // let's D3DX check the corrected pixel format
            D3D9.TextureRequirements texRequires = D3D9.Texture.CheckRequirements(d3d9Device, 0, 0, 0, 0, d3dPF,
                                                                                  this._d3dPool);
            d3dPF = texRequires.Format;

            // Use D3DX to help us create the texture, this way it can adjust any relevant sizes
            D3D9.Usage d3dUsage = (usage & TextureUsage.RenderTarget) != 0 ? D3D9.Usage.RenderTarget : 0;
            int numMips = requestedMipmapCount == (int) TextureMipmap.Unlimited ? -1 : requestedMipmapCount + 1;
            ...
            if (hwGamma)
            {
                this._hwGammaReadSupported = _canUseHardwareGammaCorrection(d3d9Device, d3dUsage,
                                                                            D3D9.ResourceType.Texture, d3dPF,
                                                                            false);
                if ((usage & TextureUsage.RenderTarget) != 0)
                {
                    this._hwGammaWriteSupported = _canUseHardwareGammaCorrection(d3d9Device, d3dUsage,
                                                                                 D3D9.ResourceType.Texture,
                                                                                 d3dPF,
                                                                                 true);
                }
            ...
            if ((usage & TextureUsage.RenderTarget) != 0)
            {
                D3D9RenderSystem rsys = (D3D9RenderSystem) Root.Instance.RenderSystem;
                rsys.DetermineFSAASettings(d3d9Device, fsaa, fsaaHint, d3dPF, false, out this._fsaaType,
                                           out this._fsaaQuality);
            }
            else
            {
            ...
            try
            {
                textureResources.NormalTexture = new D3D9.Texture(d3d9Device, SrcWidth, SrcHeight, numMips, d3dUsage,
                                                                  d3dPF,
                                                                  this._d3dPool);
            }
            catch (Exception ex)
            ...
            D3D9.SurfaceDescription desc = textureResources.NormalTexture.GetLevelDescription(0);

            if (this._fsaaType != 0)
            {
                // create AA surface
                textureResources.FSAASurface = D3D9.Surface.CreateRenderTarget(d3d9Device, desc.Width, desc.Height,
                                                                               d3dPF,
                                                                               this._fsaaType, this._fsaaQuality, false);
            }

            _setFinalAttributes(d3d9Device, textureResources, desc.Width, desc.Height, 1,
        ...
        private void _createCubeTexture(D3D9.Device d3d9Device)
        {
            ...
            // let's D3DX check the corrected pixel format
            D3D9.CubeTextureRequirements texRequires = D3D9.CubeTexture.CheckRequirements(d3d9Device, 0, 0, 0, d3dPF,
                                                                                          this._d3dPool);
            d3dPF = texRequires.Format;
            ...
            if ((rkCurCaps.TextureCaps & D3D9.TextureCaps.MipCubeMap) != 0)
            {
                if ((usage & TextureUsage.AutoMipMap) != 0 && requestedMipmapCount != 0)
                {
                    ...
                    mipmapsHardwareGenerated = _canAutoGenMipMaps(d3d9Device, d3dUsage, D3D9.ResourceType.CubeTexture,
                                                                  d3dPF);
                    if (mipmapsHardwareGenerated)
                    {
                        d3dUsage |= D3D9.Usage.AutoGenerateMipMap;
            ...
            textureResources.CubeTexture = new D3D9.CubeTexture(d3d9Device, SrcWidth, numMips, d3dUsage, d3dPF,
                                                                this._d3dPool);
            ...
            // set final tex. attributes from tex. description
            // they may differ from the source image !!!
            D3D9.SurfaceDescription desc = textureResources.CubeTexture.GetLevelDescription(0);
        ...
        private void _createVolumeTexture(D3D9.Device d3d9Device)
        {
            ...
            // let's D3DX check the corrected pixel format
            D3D9.VolumeTextureRequirements texRequires = D3D9.VolumeTexture.CheckRequirements(d3d9Device, 0, 0, 0, 0, 0,
                                                                                              d3dPF, this._d3dPool);
            d3dPF = texRequires.Format;
            ...
            if ((rkCurCaps.TextureCaps & D3D9.TextureCaps.MipVolumeMap) != 0)
            {
                if ((Usage & TextureUsage.AutoMipMap) != 0 && requestedMipmapCount != 0)
                {
                    mipmapsHardwareGenerated = _canAutoGenMipMaps(d3d9Device, d3dUsage, D3D9.ResourceType.VolumeTexture,
                                                                  d3dPF);
                    if (MipmapsHardwareGenerated)
                    {
                        d3dUsage |= D3D9.Usage.AutoGenerateMipMap;
            ...
            // create the texture
            textureResources.VolumeTexture = new D3D9.VolumeTexture(d3d9Device, Width, Height, Depth, numMips, d3dUsage,
                                                                    d3dPF,
                                                                    this._d3dPool);
            ...
            // set final tex. attributes from tex. description
            // they may differ from the source image !!!
            D3D9.VolumeDescription desc = textureResources.VolumeTexture.GetLevelDescription(0);
        ...
        /// </summary>
        [OgreVersion(1, 7, 2)]
        private void _setFinalAttributes(D3D9.Device d3d9Device, TextureResources textureResources, int width,
                                         int height,
        ...
        private D3D9.TextureFilter _getBestFilterMethod(D3D9.Device d3d9Device)
        {
            ...
            D3D9.FilterCaps filterCaps = (D3D9.FilterCaps) 0;
        ...
        {
            D3D9.Direct3D d3d = d3d9Device.Direct3D;
            //TODO
            //if ( d3d != null )
            //    d3d.Release();
            ...
            D3D9Device device = D3D9RenderSystem.DeviceManager.GetDeviceFromD3D9Device(d3d9Device);
            D3D9.Capabilities rkCurCaps = device.D3D9DeviceCaps;
            D3D9.Format eBackBufferFormat = device.BackBufferFormat;

            // check for auto gen. mip maps support
            bool hr = d3d.CheckDeviceFormat(rkCurCaps.AdapterOrdinal, rkCurCaps.DeviceType, eBackBufferFormat,
        ...
        /// </summary>
        [OgreVersion(1, 7, 2, "some todo need to be checked")]
        private bool _canUseHardwareGammaCorrection(D3D9.Device d3d9Device, D3D9.Usage srcUsage,
                                                    D3D9.ResourceType srcType,
                                                    D3D9.Format srcFormat, bool forWriting)
        {
            ...
            // check for auto gen. mip maps support
            bool hr = d3d.CheckDeviceFormat(rkCurCaps.AdapterOrdinal, rkCurCaps.DeviceType, eBackBufferFormat, srcUsage,
                                            srcType,
                                            srcFormat);
        ...
        private void _createSurfaceList(D3D9.Device d3d9Device, TextureResources textureResources)
        {
            ...
            if (!updateOldList)
            {
                ...
                for (int face = 0; face < FaceCount; ++face)
                {
                    ...
                    {
                        D3D9HardwarePixelBuffer buffer = new D3D9HardwarePixelBuffer(bufusage, this);
            ...
            switch (TextureType)
            {
                ...
                case TextureType.TwoD:
                    ...
                    for (int mip = 0; mip <= MipmapCount; ++mip)
                    {
                        D3D9.Surface surface = textureResources.NormalTexture.GetSurfaceLevel(0);
                        ...
                        currPixelBuffer.Bind(d3d9Device, surface, textureResources.FSAASurface,
                                             this._hwGammaWriteSupported, fsaa, _name,
                ...
                case TextureType.CubeMap:
                    ...
                    for (int face = 0; face < 6; ++face)
                    {
                        for (int mip = 0; mip <= MipmapCount; ++mip)
                        {
                            D3D9.Surface surface =
                                textureResources.CubeTexture.GetCubeMapSurface((D3D9.CubeMapFace) face, mip);
                ...
                case TextureType.ThreeD:
                    ...
                    for (int mip = 0; mip <= MipmapCount; ++mip)
                    {
                        D3D9.Volume volume = textureResources.VolumeTexture.GetVolumeLevel(mip);
                        D3D9HardwarePixelBuffer currPixelBuffer = _getSurfaceAtLevel(0, mip);
        ...
        public bool UseDefaultPool()
        {
            ...
            return (Usage & TextureUsage.RenderTarget) != 0 ||
                   (Usage & TextureUsage.Dynamic) != 0 && this._dynamicTextures;
        }
        ...
        public void NotifyOnDeviceDestroy(D3D9.Device d3d9Device)
        {
            ...
            if (textureResources != null)
            {
                LogManager.Instance.Write("D3D9 device: 0x[{0}] destroy. Releasing D3D9 texture: {1}",
                                          d3d9Device.ToString(), _name);
        ...
        public void NotifyOnDeviceLost(D3D9.Device d3d9Device)
        {
            ...
            if (this._d3dPool == D3D9.Pool.Default)
            {
                ...
                if (textureResources != null)
                {
                    LogManager.Instance.Write("D3D9 device: 0x[{0}] lost. Releasing D3D9 texture: {1}",
                                              d3d9Device.ToString(), _name);
