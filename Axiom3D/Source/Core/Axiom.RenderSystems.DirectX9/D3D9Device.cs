...
namespace Axiom.RenderSystems.DirectX9
{
    ...
    public class D3D9Device
    {
        ...
        /// </summary>
        [OgreVersion(1, 7, 2790, "hides mPresentationParamsCount")] protected D3D9.PresentParameters[]
            PresentationParams;
        ...
        {
            get { return this._mapRenderWindowToResources.Count; }
        }
        ...
        {
            get { return this.PresentationParams.Length > 0 && !this.PresentationParams[0].Windowed; }
        }
        ...
        public bool IsMultihead
        {
            get
            {
                foreach (KeyValuePair<D3D9RenderWindow, RenderWindowResources> it in this._mapRenderWindowToResources)
                {
        ...
        {
            get { return this._mapRenderWindowToResources.First(x => x.Value.PresentParametersIndex == 0).Key; }
        }
        ...
        public bool IsAutoDepthStencil
        {
            get
            {
                D3D9.PresentParameters primaryPresentationParams = this.PresentationParams[0];
                ...
                for (int i = 1; i < this.PresentationParams.Length; i++)
                {
                    ...
                    if (primaryPresentationParams.BackBufferHeight != this.PresentationParams[i].BackBufferHeight ||
                        ...
                        primaryPresentationParams.AutoDepthStencilFormat !=
                        this.PresentationParams[i].AutoDepthStencilFormat ||
        ...
        public bool IsDeviceLost
        {
            get
            {
                DX.Result hr = this.pDevice.TestCooperativeLevel();
        ...
        [OgreVersion(1, 7, 2790)]
        public D3D9Device(D3D9DeviceManager d3D9DeviceManager, int adapterNumber, IntPtr hMonitor,
                          D3D9.DeviceType devType,
            ...
            D3D9RenderWindow renderWindow)
        {
            ...
            return new KeyValuePair<D3D9RenderWindow, RenderWindowResources>(renderWindow,
                                                                             this._mapRenderWindowToResources[
                                                                                 renderWindow]);
        }
        ...
        public bool Acquire()
        {
            ...
            else
            {
                ...
                {
                    D3D9.PresentParameters currentPresentParams = itPrimary.SwapChain.PresentParameters;
        ...
        protected void UpdatePresentationParameters()
        {
            ...
            if (this._mapRenderWindowToResources.Count > 0)
            {
                ...
                foreach (KeyValuePair<D3D9RenderWindow, RenderWindowResources> it in this._mapRenderWindowToResources)
                {
                    ...
                    {
                        this.PresentationParams[renderWindowResources.PresentParametersIndex] =
        ...
        public void ClearDeviceStreams()
        {
            ...
            {
                DX.Result hr = this.pDevice.SetTexture(stage, null);
                if (hr.Failure)
                {
                    throw new AxiomException("Unable to disable texture '{0}' in D3D9", stage);
                }

                D3D9.TextureOperation dwCurValue = this.pDevice.GetTextureStageState<D3D9.TextureOperation>(stage,
                                                                                                            D3D9.
                                                                                                                TextureStage
                                                                                                                .
                                                                                                                ColorOperation);
                ...
                {
                    hr = this.pDevice.SetTextureStageState(stage, D3D9.TextureStage.ColorOperation,
                                                           D3D9.TextureOperation.Disable);
                    if (hr.Failure)
                    {
                        throw new AxiomException("Unable to disable texture '{0}' in D3D9", stage);
                    }
        ...
        protected void CreateD3D9Device()
        {
            ...
            D3D9.Direct3D pD3D9 = D3D9RenderSystem.Direct3D9;
            ...
            while (keepTrying)
            {
                try
                {
                    this.pDevice = new D3D9.Device(pD3D9, AdapterNumber, DeviceType, this._focusWindow,
                                                   this.BehaviorFlags,
        ...
        protected void AcquireRenderWindowResources(KeyValuePair<D3D9RenderWindow, RenderWindowResources> it)
        {
            ...
            if (IsSwapChainWindow(renderWindow) && !IsMultihead)
            {
                ...
                {
                    // Create swap chain
                    renderWindowResources.SwapChain = new D3D9.SwapChain(this.pDevice,
                                                                         renderWindowResources.PresentParameters);
                }
            ...
            if (renderWindow.IsDepthBuffered)
            {
                ...
                else
                {
                    ...
                    renderWindowResources.DepthBuffer = D3D9.Surface.CreateDepthStencil(this.pDevice, targetWidth,
                                                                                        targetHeight,
        ...
        protected void UpdateRenderWindowsIndices()
        {
            ...
            else
            {
                ...
                {
                    foreach (
                        KeyValuePair<D3D9RenderWindow, RenderWindowResources> it in this._mapRenderWindowToResources)
                    {
        ...
        public void CopyContentsToMemory(D3D9RenderWindow renderWindow, PixelBox dst, RenderTarget.FrameBuffer buffer)
        {
            ...
            D3D9.SurfaceDescription desc = new D3D9.SurfaceDescription();
            ...
            {
                D3D9.DisplayMode dm = this.pDevice.GetDisplayMode(0);
                ...
                if (renderWindow.IsFullScreen)
                {
                    try
                    {
                        ...
                        {
                            Rectangle rect = new System.Drawing.Rectangle(dst.Left, dst.Top, dst.Right - dst.Left,
                                                                          dst.Bottom - dst.Top);
                            lockedRect = pTempSurf.LockRectangle(rect,
                                                                 D3D9.LockFlags.ReadOnly | D3D9.LockFlags.NoSystemLock);
                        }
                    }
                    catch
                    {
                ...
                else
                {
                    ...
                    Rectangle srcRect = new System.Drawing.Rectangle(dst.Left, dst.Top, dst.Right - dst.Left,
                                                                     dst.Bottom - dst.Top);
                    Point point = new System.Drawing.Point(srcRect.Left, srcRect.Top);
                    ...
                    try
                    {
                        lockedRect = pTempSurf.LockRectangle(srcRect,
                                                             D3D9.LockFlags.ReadOnly | D3D9.LockFlags.NoSystemLock);
                    }
                    catch
                    {
            ...
            else
            {
                ...
                {
                    DX.Result hr = this.pDevice.GetRenderTargetData(pSurf, pTempSurf);
                    if (hr.Failure)
                    {
                        pTempSurf.SafeDispose();
                ...
                {
                    D3D9.Surface pStretchSurf = D3D9.Surface.CreateRenderTarget(this.pDevice, desc.Width, desc.Height,
                                                                                desc.Format,
                                                                                D3D9.MultisampleType.None, 0, false);
                    DX.Result hr = this.pDevice.StretchRectangle(pSurf, pStretchSurf, D3D9.TextureFilter.None);
                    if (hr.Failure)
                    {
                        pTempSurf.SafeDispose();
            ...
            {
                PixelBox src = new PixelBox(dst.Width, dst.Height, 1, format, data);
