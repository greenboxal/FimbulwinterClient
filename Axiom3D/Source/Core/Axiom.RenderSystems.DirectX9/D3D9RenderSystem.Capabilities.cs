...
//     <id value="$Id: D3D9RenderSystem.Capabilities.cs 3293 2012-05-21 11:56:22Z borrillis $"/>
// </file>
    ...
    public partial class D3D9RenderSystem
    {
        ...
        private RenderSystemCapabilities _updateRenderSystemCapabilities(D3D9RenderWindow renderWindow)
        {
            ...
            foreach (D3D9Device device in this._deviceManager)
            {
                ...
                D3D9.Surface pSurf = d3D9Device.DepthStencilSurface;
                if (pSurf != null)
                {
                    D3D9.SurfaceDescription surfDesc = pSurf.Description;
                    //TODO
                    //pSurf.Release();
            ...
            {
                D3D9.Capabilities rkCurCaps = pCurDriver.D3D9DeviceCaps;
                ...
                {
                    rsc.MultiRenderTargetCount = Utility.Min(rkCurCaps.SimultaneousRTCount,
            ...
            // Adapter details
            D3D9.AdapterDetails adapterId = this._activeD3DDriver.AdapterIdentifier;
            ...
            D3D9.Format[] floatFormats = new[]
                                             {
                                                 D3D9.Format.R16F, D3D9.Format.G16R16F, D3D9.Format.A16B16G16R16F,
                                                 D3D9.Format.R32F,
                                                 D3D9.Format.G32R32F, D3D9.Format.A32B32G32R32F
                                             };

            D3D9.Surface[] bbSurf = (D3D9.Surface[]) renderWindow["DDBACKBUFFER"];
            D3D9.SurfaceDescription bbSurfDesc = bbSurf[0].Description;

            for (int i = 0; i < 6; ++i)
            {
                if (
                    !this._pD3D.CheckDeviceFormat(this._activeD3DDriver.AdapterNumber, D3D9.DeviceType.Hardware,
                                                  bbSurfDesc.Format, 0,
            ...
            if (rsc.IsShaderProfileSupported("ps_3_0"))
            {
                ...
                switch (rsc.Vendor)
                {
                    case GPUVendor.Nvidia:
                        if (this._pD3D.CheckDeviceFormat(0, D3D9.DeviceType.Hardware, D3D9.Format.X8R8G8B8, 0,
                                                         D3D9.ResourceType.Surface,
        ...
        private bool _checkVertexTextureFormats(D3D9RenderWindow renderWindow)
        {
            ...
            {
                D3D9.Format fmt = D3D9Helper.ConvertEnum(D3D9Helper.GetClosestSupported(pf));
                if (
                    !this._pD3D.CheckDeviceFormat(this._activeD3DDriver.AdapterNumber, D3D9.DeviceType.Hardware,
                                                  bbSurfDesc.Format,
        ...
        private void _convertPixelShaderCaps(RenderSystemCapabilities rsc)
        {
            int major = 0xFF;
            int minor = 0xFF;
            D3D9.Capabilities minPsCaps = new D3D9.Capabilities();

            // Find the device with the lowest vertex shader caps.
            foreach (D3D9Driver pCurDriver in this._driverList)
            {
                D3D9.Capabilities currCaps = pCurDriver.D3D9DeviceCaps;
            ...
            if (major >= 2)
            {
                ...
                if ((minPsCaps.PS20Caps.Caps & D3D9.PixelShaderCaps.NoTextureInstructionLimit) != 0 &&
                    ...
                    (minPsCaps.PS20Caps.Caps & D3D9.PixelShaderCaps.Predication) != 0 &&
        ...
        private void _convertVertexShaderCaps(RenderSystemCapabilities rsc)
        {
            int major = 0xFF;
            int minor = 0xFF;
            D3D9.Capabilities minVsCaps = new D3D9.Capabilities();
