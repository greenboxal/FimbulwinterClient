...
namespace Axiom.RenderSystems.DirectX9
{
    ...
    public class D3D9HardwarePixelBuffer : HardwarePixelBuffer
    {
                         ...
                         string srcName, D3D9.BaseTexture mipTex)
        {
            ...
            D3D9.SurfaceDescription desc = surface.Description;
            width = desc.Width;
            height = desc.Height;
            depth = 1;
            ...
            {
                foreach (KeyValuePair<D3D9.Device, BufferResources> it in this.mapDeviceToBufferResources)
                {
                    if (it.Value != bufferResources && it.Value.Surface != null && it.Key.TestCooperativeLevel().Success &&
                        dev.TestCooperativeLevel().Success)
                    {
                        BasicBox fullBufferBox = new BasicBox(0, 0, 0, Width, Height, Depth);
                        PixelBox dstBox = new PixelBox(fullBufferBox, Format);
        ...
        public void Bind(D3D9.Device dev, D3D9.Volume volume, D3D9.BaseTexture mipTex)
        {
            ...
            D3D9.VolumeDescription desc = volume.Description;
            width = desc.Width;
            height = desc.Height;
            depth = desc.Depth;
            format = D3D9Helper.ConvertEnum(desc.Format);
        ...
        protected static System.Drawing.Rectangle ToD3DRectangle(BasicBox lockBox)
        {
            ...
            Rectangle r = new System.Drawing.Rectangle();
            r.X = lockBox.Left;
        ...
        protected override PixelBox LockImpl(BasicBox lockBox, BufferLocking options)
        {
            ...
            D3D9.LockFlags flags = D3D9Helper.ConvertEnum(options, usage);
        ...
        protected PixelBox LockBuffer(BufferResources bufferResources, BasicBox lockBox, D3D9.LockFlags flags)
        {
            ...
            PixelBox rval = new PixelBox(lockBox.Width, lockBox.Height, lockBox.Depth, Format);
            ...
            {
                // Volume
                D3D9.Box pbox = ToD3DBox(lockBox); // specify range to lock
                DX.DataBox lbox = bufferResources.Volume.LockBox(pbox, flags);
        ...
        public override void Blit(HardwarePixelBuffer rsrc, BasicBox srcBox, BasicBox dstBox)
        {
            ...
            D3D9HardwarePixelBuffer _src = (D3D9HardwarePixelBuffer) rsrc;
            foreach (KeyValuePair<D3D9.Device, BufferResources> it in this.mapDeviceToBufferResources)
            {
                            ...
                            BufferResources srcBufferResources, BufferResources dstBufferResources)
        {
            if (dstBufferResources.Surface != null && srcBufferResources.Surface != null)
            {
                ...
                D3D9.SurfaceDescription srcDesc = srcBufferResources.Surface.Description;
                ...
                {
                    // Temp texture
                    D3D9.Texture tmptex = new D3D9.Texture(d3d9Device, srcDesc.Width, srcDesc.Height, 1,
                                                           // 1 mip level ie topmost, generate no mipmaps
                                                           0, srcDesc.Format, D3D9.Pool.SystemMemory);

                    D3D9.Surface tmpsurface = tmptex.GetSurfaceLevel(0);

                    if (d3d9Device.GetRenderTargetData(srcBufferResources.Surface, tmpsurface).Success)
                    {
                        // Hey, it worked
                        // Copy from this surface instead
                        DX.Result res = D3D9.Surface.FromSurface(dstBufferResources.Surface, tmpsurface,
                                                                 D3D9.Filter.Default, 0, dsrcRect,
                                                                 ddestRect);
                        if (res.Failure)
                        {
                ...
                DX.Result res2 = D3D9.Surface.FromSurface(dstBufferResources.Surface, srcBufferResources.Surface,
            ...
            {
                // Volume-to-volume
                D3D9.Box dsrcBox = ToD3DBox(srcBox);
                D3D9.Box ddestBox = ToD3DBox(dstBox);

                DX.Result res = D3D9.Volume.FromVolume(dstBufferResources.Volume, srcBufferResources.Volume,
                                                       D3D9.Filter.Default, 0,
                                                       dsrcBox, ddestBox);
                if (res.Failure)
                {
        ...
        protected void BlitFromMemory(PixelBox src, BasicBox dstBox, BufferResources dstBufferResources)
        {
            ...
            if (dstBufferResources.Surface != null)
            {
                ...
                {
                    throw new AxiomException(
                        "D3D9.Surface.FromMemory failed in D3D9HardwarePixelBuffer.BlitFromMemory", e);
                }
            }
            else if (dstBufferResources.Volume != null)
            {
                D3D9.Box srcBox = ToD3DBox(converted);
                D3D9.Box destBox = ToD3DBox(dstBox);
                int sliceWidth = 0;
                if (PixelUtil.IsCompressed(converted.Format))
                {
                ...
                try
                {
                    using (BufferBase srcData = BufferBase.Wrap(data))
                    {
                        IntPtr srcMemoryPtr = new IntPtr(srcData.Ptr);
                        dstBufferResources.Volume.LoadFromMemory(null, destBox, srcMemoryPtr,
                                                                 D3D9Helper.ConvertEnum(converted.Format),
                                                                 rowWidth, slicePitch, null, srcBox, D3D9.Filter.Default,
                                                                 0);
                    }
                }
                catch (Exception e)
                {
                    throw new AxiomException(
                        "D3D9.Volume.FromFileInMemory failed in D3D9HardwarePixelBuffer.BlitFromMemory", e);
                }
            }
        ...
        public override void BlitToMemory(BasicBox srcBox, PixelBox dst)
        {
            ...
            KeyValuePair<D3D9.Device, BufferResources> pair = this.mapDeviceToBufferResources.First();
        ...
        [OgreVersion(1, 7, 2)]
        protected void BlitToMemory(BasicBox srcBox, PixelBox dst, BufferResources srcBufferResources,
                                    D3D9.Device d3d9Device)
        {
            ...
            if (srcBufferResources.Surface != null)
            {
                ...
                D3D9.SurfaceDescription srcDesc = srcBufferResources.Surface.Description;
                D3D9.Pool temppool = D3D9.Pool.Scratch;
                ...
                // Create temp texture
                D3D9.Texture tmp = new D3D9.Texture(d3d9Device, dst.Width, dst.Height, 1,
                                                    // 1 mip level ie topmost, generate no mipmaps
                                                    0, D3D9Helper.ConvertEnum(tmpFormat), temppool);

                D3D9.Surface surface = tmp.GetSurfaceLevel(0);
                ...
                D3D9.SurfaceDescription dstDesc = surface.Description;
                ...
                {
                    DX.Result result = d3d9Device.GetRenderTargetData(srcBufferResources.Surface, surface);
                    fastLoadSuccess = result.Success;
                }
                if (!fastLoadSuccess)
                {
                    DX.Result res = D3D9.Surface.FromSurface(surface, srcBufferResources.Surface, D3D9.Filter.Default, 0,
                                                             srcRect, destRect);
                    if (res.Failure)
                    {
                        surface.SafeDispose();
                        tmp.SafeDispose();
                        throw new AxiomException(
                            "D3D9.Surface.FromSurface failed in D3D9HardwarePixelBuffer.BlitToMemory");
                    }
                }

                // Lock temp surface and copy it to memory
                DX.DataRectangle lrect = surface.LockRectangle(D3D9.LockFlags.ReadOnly);
            ...
            {
                // Create temp texture
                D3D9.VolumeTexture tmp = new D3D9.VolumeTexture(d3d9Device, dst.Width, dst.Height, dst.Depth, 0, 0,
                                                                D3D9Helper.ConvertEnum(tmpFormat), D3D9.Pool.Scratch);

                D3D9.Volume surface = tmp.GetVolumeLevel(0);

                // Volume
                D3D9.Box ddestBox = ToD3DBoxExtent(dst);
                D3D9.Box dsrcBox = ToD3DBox(srcBox);

                DX.Result res = D3D9.Volume.FromVolume(surface, srcBufferResources.Volume, D3D9.Filter.Default, 0,
                                                       dsrcBox, ddestBox);
                if (res.Failure)
                {
                    surface.SafeDispose();
                ...
                // Lock temp surface and copy it to memory
                DX.DataBox lbox = surface.LockBox(D3D9.LockFlags.ReadOnly); // Filled in by D3D
