...
//     <id value="$Id: D3D9HardwareIndexBuffer.cs 3361 2012-07-29 00:06:09Z romeoxbm $"/>
// </file>
    ...
    public sealed class D3D9HardwareIndexBuffer : HardwareIndexBuffer, ID3D9Resource
    {
        ...
        public D3D9.IndexBuffer D3DIndexBuffer
        {
            get
            {
                ...
                bool wasBufferFound = this._mapDeviceToBufferResources.TryGetValue(d3d9Device, out resources);
            ...
            : base(manager, type, numIndices, usage, useSystemMemory, useShadowBuffer)
        {
            ...
            D3D9.Pool eResourcePool = useSystemMemory
        ...
        protected override BufferBase LockImpl(int offset, int length, BufferLocking options)
        {
            ...
            {
                foreach (KeyValuePair<D3D9.Device, BufferResources> it in this._mapDeviceToBufferResources)
                {
                    ...
                    {
                        int highPoint = Math.Utility.Max(offset + length,
        ...
        public void CreateBuffer(D3D9.Device d3d9Device, D3D9.Pool ePool)
        {
            ...
            try
            {
                bufferResources.IndexBuffer = new D3D9.IndexBuffer(d3d9Device, sizeInBytes,
                                                                   D3D9Helper.ConvertEnum(usage), ePool,
        ...
        private bool _updateBufferResources(BufferBase systemMemoryBuffer, ref BufferResources bufferResources)
        {
            ...
            Contract.RequiresNotNull(bufferResources.IndexBuffer,
                                     "Cannot update BufferResources in D3D9HardwareIndexBuffer!");
            ...
            using (BufferBase src = ((BufferBase) systemMemoryBuffer.Clone()).Offset(bufferResources.LockOffset))
            {
                using (BufferBase dest = BufferBase.Wrap(dstBytes.DataPointer, bufferResources.LockLength))
                {
            ...
            // Unlock the buffer.
            DX.Result hr = bufferResources.IndexBuffer.Unlock();
            if (hr.Failure)
            {
