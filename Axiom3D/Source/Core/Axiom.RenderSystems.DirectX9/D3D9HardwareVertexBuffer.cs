...
//     <id value="$Id: D3D9HardwareVertexBuffer.cs 3361 2012-07-29 00:06:09Z romeoxbm $"/>
// </file>
...
namespace Axiom.RenderSystems.DirectX9
{
    ...
    public sealed class D3D9HardwareVertexBuffer : HardwareVertexBuffer, ID3D9Resource
    {
        ...
        protected override BufferBase LockImpl(int offset, int length, BufferLocking options)
        {
            ...
            if (options != BufferLocking.ReadOnly)
            {
                foreach (KeyValuePair<D3D9.Device, BufferResources> it in this._mapDeviceToBufferResources)
                {
                    ...
                    {
                        int highPoint = Utility.Max(offset + length,
        ...
        public void CreateBuffer(D3D9.Device d3d9Device, D3D9.Pool ePool)
        {
            ...
            try
            {
                bufferResources.VertexBuffer = new D3D9.VertexBuffer(d3d9Device, sizeInBytes,
                                                                     D3D9Helper.ConvertEnum(usage), 0, ePool);
            }
            catch (Exception ex)
            {
        ...
        private bool _updateBufferResources(BufferBase systemMemoryBuffer, ref BufferResources bufferResources)
        {
            ...
            Contract.RequiresNotNull(bufferResources.VertexBuffer,
                                     "Cannot update BufferResources in D3D9HardwareVertexBuffer!");
            ...
            // Unlock the buffer.
            DX.Result hr = bufferResources.VertexBuffer.Unlock();
            if (hr.Failure)
            {
