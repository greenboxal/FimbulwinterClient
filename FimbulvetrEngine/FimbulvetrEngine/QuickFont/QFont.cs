...
namespace QuickFont
{
    ...
    public class QFont
    {
        ...
        public QFont(IntPtr data, int datasize, float size, FontStyle style, QFontBuilderConfiguration config)
        {
            PrivateFontCollection pfc = new PrivateFontCollection();
            pfc.AddMemoryFont(data, datasize);
            var fontFamily = pfc.Families[0];

            if (!fontFamily.IsStyleAvailable(style))
                throw new ArgumentException("Font does not support style: " +  style );
